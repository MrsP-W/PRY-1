...
def dashboard_ctx_with_real_note_confirm() -> Any:
    ...
    master = b"z" * 32
    with sf() as session:
        # 写 2 条全明文 + 1 条手动 enc:v3: 密文,验证混合场景
        session.add(
            Note(
        ...
        # 手动预加密(用相同 master_key + NotesCipherImpl 写 enc:v3: ... 形式)
        ...
        # impl.encrypt 返回的密文必须是当前写入格式 `enc:v3:`。
        ...
        assert enc_body.startswith(_CIPHERTEXT_PREFIX_V3)
        session.add(
            Note(
...
def test_build_notes_pending_payload_decrypts_real_encrypted_notes(
    ...
    库内密文(enc:v3:)和明文混存,items[].title 全部出库为明文(撞坑 #65 兼容)."""
    from my_ai_employee.core.notes_encryption import is_notes_ciphertext
