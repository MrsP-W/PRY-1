"""D6.1 微信账单 CSV 多版本解析器测试(8 cases).

承接 docs/v0.1-launch-plan.md §D6 + plan §3 D6.1:
...
跑法:
    pytest tests/connectors/test_wechat_csv.py -v
"""
...
# Faker 样本目录(沿 plan §3 D6.1 fixtures)
_FAKER_DIR = Path(__file__).resolve().parent.parent / "fixtures" / "wechat_faker"
...
def test_2024_parser_5_rows_all_fields() -> None:
    ...
    """
    from my_ai_employee.connectors.wechat_csv import WeChatCSV2024Parser

    path = _FAKER_DIR / "wechat_2024_sample.csv"
    ...
    assert results[0].external_transaction_id == "42000001202405121430001"
...
def test_2025_parser_5_rows_all_fields() -> None:
    ...
    from my_ai_employee.connectors.wechat_csv import WeChatCSV2025Parser

    path = _FAKER_DIR / "wechat_2025_sample.csv"
    ...
    # 2025 字段名差异: 收/付(不用"交易类型"), 交易单号(不用"交易号")
    assert results[0].counterparty == "麦当劳(朝阳店)"
    assert results[0].amount == Decimal("42.00")
    assert results[0].type == "支出"  # 收/付 = 付 → 支出(沿 _normalize_type 范本)
    assert results[0].external_transaction_id == "42000001202503101000001"
...
def test_2026_parser_raises_not_implemented() -> None:
    ...
    from my_ai_employee.connectors.wechat_csv import WeChatCSV2026Parser

    parser = WeChatCSV2026Parser()
    assert parser.version == 2026

    path = _FAKER_DIR / "wechat_2026_sample.csv"
    with pytest.raises(NotImplementedError, match="2026 微信账单 CSV 字段待用户真实样本补充"):
        list(parser.parse(path))


def test_detect_version_3_versions() -> None:
    """Case 4 — detect_version 对 3 版本样本命中率 100%."""
    from my_ai_employee.connectors.wechat_csv import detect_version

    assert detect_version(_FAKER_DIR / "wechat_2024_sample.csv") == 2024
    assert detect_version(_FAKER_DIR / "wechat_2025_sample.csv") == 2025
    assert detect_version(_FAKER_DIR / "wechat_2026_sample.csv") == 2026
...
def test_detect_version_unsupported_raises() -> None:
    ...
    from my_ai_employee.connectors.wechat_csv import UnsupportedCSVVersionError, detect_version

    with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False, encoding="utf-8") as f:
        f.write("foo,bar,baz\n1,2,3\n")
        unknown_path = Path(f.name)
    try:
        with pytest.raises(UnsupportedCSVVersionError, match="无法嗅探微信账单版本"):
...
def test_get_parser_invalid_version_guards() -> None:
    ...
    from my_ai_employee.connectors.wechat_csv import (
