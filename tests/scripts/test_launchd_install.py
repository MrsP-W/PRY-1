...
def test_p1_5_news_refresh_plist_is_safe_hourly_one_shot() -> None:
    """P1.5:新闻刷新独立运行，每小时执行且失败不由 KeepAlive 热循环。"""
    assert PLIST_NEWS_REFRESH_PATH.exists()
    with PLIST_NEWS_REFRESH_PATH.open("rb") as f:
        data = plistlib.load(f)

    assert data["Label"] == "com.myaiemployee.news-refresh"
    assert data["ProgramArguments"] == ["/Users/$USER/bin/my-ai-employee-news-refresh-runner"]
    assert data["RunAtLoad"] is True
...
def test_p1_5_news_wrapper_uses_absolute_refresh_script_without_service_controls() -> None:
    """P1.5:新闻 one-shot 在 launchd HOME CWD 下仍安全调用绝对脚本路径。"""
    text = INSTALL_SH.read_text(encoding="utf-8")
    start = text.index('cat << EOF > "${TARGET_NEWS_REFRESH_WRAPPER}"')
    end = text.index('chmod +x "${TARGET_NEWS_REFRESH_WRAPPER}"')
    wrapper = text[start:end]
...
@pytest.mark.parametrize(
    ("registered_label", "expects_unload"),
    (
        ("com.myaiemployee.menu-bar", True),
        ("comXmyaiemployeeYmenu-bar", False),
        ("com.myaiemployee.menu-bar-backup", False),
    ),
)
def test_k8_uninstall_unloads_only_exact_registered_label(
    tmp_path: Path, registered_label: str, expects_unload: bool
) -> None:
    """K8. uninstall 只能卸载精确 label，不能把近似 label 当作已注册 job。"""
    home = tmp_path / "home"
    fake_bin = tmp_path / "fake-bin"
    fake_bin.mkdir()
    launchctl_state = tmp_path / "launchctl-state"
    ...
    assert result.returncode == 0, (
        f"uninstall 在 {registered_label} 场景必须完成,"
        f"stdout={result.stdout}\nstderr={result.stderr}"
    )
    calls = launchctl_calls.read_text(encoding="utf-8") if launchctl_calls.exists() else ""
    if expects_unload:
        expected = f"unload {home}/Library/LaunchAgents/{registered_label}.plist\n"
        assert calls == expected, f"精确 label 必且只能 unload 自身,实际={calls}"
        assert launchctl_state.read_text(encoding="utf-8") == "", "精确 label 必被 fake unload 清除"
    else:
        assert calls == "", f"近似 label 不得触发 unload 或 bootout,实际={calls}"
        assert launchctl_state.read_text(encoding="utf-8") == f"123 0 {registered_label}\n", (
            "近似 label 必保持未触碰"
        )
