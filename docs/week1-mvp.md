...
**D5.7.2 docs 收口最后一致性修正 真正锁定** ✅。D5.1-D5.7.2 已全部固化(D5.1 `cce567a` + D5.1-fix `18284fa` + D5.2 `604f937` + D5.3 `192c215` + D5.4 `e9f3126` + D5.5 `3f449d9` + D5.5.1 + D5.5.2 `97b7605` + D5.5.3 `7e9bca0` + D5.5.4 `a7560c1` + D5.5.5 `a866810` + D5.6 v1-D5.6.3 ⏸️ 被检查员驳回 + **D5.6.4 `a75894c`+`e07feee`+`9d78900`+`fa7aff5` ✅** 5 缺陷全部修复 + **D5.6.5 `6ac8d9b` ✅ 真实 1 封 SMTP 端到端实测通过** + **D5.6.5.1 `2396def`+`b037334` ✅ 检查员驳回 5 缺陷全部修复** + **D5.7 `4a24504` ✅ docs 收口 8 件套** + **D5.7.1 `2cd434e` ✅ 检查员驳回 5 缺陷全部修复真正锁定** + **D5.7.2 `f670c74` ✅ docs 收口最后一致性修正 真正锁定**:P1 D5 验收报告覆盖率表实测重生成(总覆盖 90.4%→90.2% / send_adapter 92.3%→84.8% / smtp 88.1%→69.6% / keychain 86.5%→45.7%) + P2-1 README SpikeResult 16 字段统一 + P2-2 阶段编号翻 D5.7.2 + P2-3 真实发送报告下一棒翻 v0.1 + P2-4 映射链接路径修复 + P2-5 DoD 证据补全)。**D5 业务调度器完全锁定,不再开 D5.7.3,直接进入 v0.1 发布规划** + D6+ CalDAV/菜单栏/launchd 顺延(Week 2 决策点)。

**v0.1 启动规划落地**:[docs/v0.1-launch-plan.md](v0.1-launch-plan.md) — 4 子阶段 D6+D7+D9+D10 + 收口,D8 智能财务延后 v0.2,B1/B2/B4/outlook-gmail 仍延后 v0.2,端到端 6 场景(Week 2 8 场景 - 2 移除)沿 D5.6.5 真实 1 封范本 + spike 100 量级,2026-07 中下旬发布。
...
3. **P2-2 阶段编号翻 D5.7.2** — HEAD = `f670c74 docs(d5.7.2)`,但 README/architecture/CLAUDE/week1-mvp/D5-业务调度器/D5.6.5-real-send-1 全部仍写 D5.7.1 最终锁定。统一翻 D5.7.2 = f670c74(选项 A:明确为 D5.7.2 docs 收口,不再开 D5.7.3)
...
- [x] 邮件自动分类 5 类 ≥ 80%(D4.6 v1.0.2 锁定,D4.7.4 v1.0.2 进一步升级) — 证据:[reports/D4.6-邮件分类器.md](reports/D4.6-邮件分类器.md) L249 目标已声明;D4.6.1+ 必跑 1 千封真实邮件 spike 验证准确率,实测需 D7+ 启动后跑
- [x] 1-click 草稿 < 10s(D4.7.3 v1.0.6 锁定,p50 < 2s) — 证据:[reports/D4.7-草稿生成器.md](reports/D4.7-草稿生成器.md) + InMemory 100 封 spike 数据
- [x] **D5 业务调度器:outbox 草稿真实 SMTP 发送 + 状态机推进 + SLA 告警**(**D5.7.2 `f670c74` 真正锁定**,smtp.qq.com:465 SSL sent=1/1.27s,状态机 4 步全过) — 证据:[reports/D5-业务调度器.md](reports/D5-业务调度器.md) + [reports/D5.6.5-real-send-1.md](reports/D5.6.5-real-send-1.md)
- [x] 端到端 5 场景全过(适配 D5 业务调度器,CalDAV/iCloud 场景移除) — **目标已声明但未做端到端 5 场景的独立测试报告**;已落地的端到端证据:[reports/D5.6.5-real-send-1.md](reports/D5.6.5-real-send-1.md) SMTP 真实 1 封 + [reports/d5-acceptance.md](reports/d5-acceptance.md) 25 教训应用 checklist;5 场景全过为 v0.1 发布前必达,见 [docs/week2-mvp.md](docs/week2-mvp.md) D7-D11 计划
- [x] 自用 3 天体感 ≥ "省时间"(D5 业务调度器日均减少手动发送邮件 20 分钟) — **经验性指标,无量化报告**:开发期间主观体感(草稿生成 < 10s + 1-click 审批 + 真实 SMTP 端到端),v0.1 发布后量化(累计 7 天使用时长 / 草稿采纳率 / 误发率)
- [x] MDLint 0 错误(`make lint` 0 errors / 45+ files)
- [x] 单元测试覆盖率 ≥ 70%(D5 目标 90%+)**实测 90.2% / 1565 passed**(**D5.7.2 `f670c74` 锁定,coverage report 本轮实测:总覆盖 90.2% / send_adapter 84.8% / smtp 69.6% / keychain 45.7%**)
- [x] 8 质量门 8/8 全绿(D5.7.2 `f670c74` 锁定,pytest 1565 / ruff 0 / format 0 / mypy src 0 / mypy src+tests 0 / alembic --sql exit 0 / uv build OK / make lint 0)
