...
>
> **承接 D4.7.3 v1.0.6 + D4.7.4 v1.0.2 + D4.8 v1.0.1 范本**:三入口架构(成功/业务阻断/技术失败) + 工厂层 + `__post_init__` 双层防御 + 25 教训 + 7 项核心契约全应用,在 D5 第一个真实业务场景(SMTP 真实发送)上**复用 + 进化**(增加 SENDING 中间态 + APPROVED 收窄 + 4 重防误发 + SMTP_REAL_NETWORK env 门 + SpikeResult 16 字段(D5.7.1 P2-3 统一))。
...
- **event_fingerprint 幂等性**:claw-code `compute_event_fingerprint` 范本 → D5 `SpikeResult` 16 字段结构化结果(下游可序列化,D5.7.1 P2-3 统一)
...
- **不写好但不返回值给 caller(D5.6.5.1 P2-1 范本)**:`SpikeResult` 16 字段 dataclass 定义完整但 `run_spike` 必须真正返回(16 字段),下游才能消费
...
- **不保留真实凭据明文(D5.6.5.1 P1-2 范本)**:`477***009@qq.com`(脱敏)完整邮箱在 5 处文档泄露,全量替换 `477***009@qq.com`(脱敏) + grep 验证 0 处残留 + 跨项目 memory 同步
...
12. **D5.6.5.1 P2-1** SpikeResult 16 字段(D5.7.1 P2-3 统一)真正落地(`-> SpikeResult` + 末尾构造并返回)
...
- **D5.6.5.1 P1-2 邮箱脱敏**:5 文件全量 `477***009@qq.com`(脱敏) → `477***009@qq.com`(脱敏)
- **D5.6.5.1 P2-1 SpikeResult 真正落地**:`run_spike(...) -> SpikeResult` + 末尾构造并返回 16 字段(D5.7.1 P2-3 统一)
...
| `SpikeResult` 定义完整但 `run_spike` 返 None(D5.6.5 之前) | `-> SpikeResult` + 末尾构造并返回 16 字段(D5.6.5.1 收口,D5.7.1 P2-3 统一) | dataclass 定义必配套 return |
...
- `memory/d5.5.2-p1-fixes.md` — D5.5.2 P1 修复(实际位于全局 memory,本项目级未同步副本)
- `memory/d5.5.1-fix-locked.md` — D5.5.1 落锁(实际位于全局 memory,本项目级未同步副本)
- `memory/d5.5-audit-findings.md` — D5.5 审计发现 7 处(实际位于全局 memory,本项目级未同步副本)
...
- [memory/tools_status.md](../../Agent%20Assistant/memory/tools_status.md) — gh api 旁路 GFW 用法
- `memory/d4.7.4-v1.0.3-deferred.md` — D4.7.4 spike 3 FALSE_PASS 列入 v1.0.3 改进项(B 类延后,实际位于全局 memory,本项目级未同步副本)
- `memory/d4.7.3-drafter-adapter-v1.0.6.md` — D4.7.3 25 教训沉淀源头(D4.8 7 项核心契约复用,实际位于全局 memory,本项目级未同步副本)
- `memory/d4.7.4-docs-closure.md` — D4.7.4 v1.0.2 docs-only 收口 commit `b1497b3`(实际位于全局 memory,本项目级未同步副本)
