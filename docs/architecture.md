...
>
> **状态**：D1-D7 + D9.1-D9.6 + S6+S7 e2e 全部实化收口(2026-06-15:smtp.qq.com:465 SSL 真实 1 封 SMTP 端到端实测通过 + D6 微信账单 + D7 支付宝账单/跨源去重 + D9.1 Apple Notes 适配器 + D9.2 sync_notes.py CLI + D9.3 rumps 菜单栏骨架 + D9.4 NoteStructurerService 3 入口 + D9.5 ⌥⌘N 双进程范本 + S6+S7 端到端 e2e 实化 + **D9.6 5 修复(ClipboardCaptureService 3 入口 / AppleScript ASCII 30 协议 / sync_notes OperationalError 透传 → exit 3 / T11 队列真调 / coverage fail_under=80)** 全部落地;**1858 passed / 5 skipped / coverage 89.5% / 9 质量门 9/9 全绿**;HEAD `9afebfe` ← `4dd47a7` ← `abe3d96` ← `21b9b90` ← `9ce1367` ← `0b2c5d0` ← `762700c` ← `ee70acb` ← `d0d3966` ← `a34f20f` ← `bfa48aa` ← `98d9d64` ← `87e16c2` ← `32fa260` ← `1ec2caf` 13 commits 收口链)。**D5 业务调度器完全锁定 + D9 Apple Notes 全链路实化,直接进入 v0.1 发布规划中段**:[docs/v0.1-launch-plan.md](v0.1-launch-plan.md) — 4 子阶段 D6+D7+D9+D10 + 收口(D6/D7/D9.1-D9.6/S6/S7 e2e 已落,D10 待启动),D8 智能财务延后 v0.2,B1/B2/B4/outlook-gmail 仍延后,2026-07 中下旬发布。CalDAV / 菜单栏 / launchd 顺延 D6+。
...
| **apple_notes** | Apple Notes | AppleScript 读取(ASCII 30/31 二级分隔) | 事件触发 | Week 2 D9.1-D9.6(已落 11 commits 收口 `1ec2caf → 9afebfe`,D9.6.2 改 AppleScript 协议避逗号/竖线/英文日期坑) |
...
### 场景 4：剪贴板笔记自动结构化(D9.5 + D9.6.1 已实化,沿 ⌥⌘N 双进程范本)
...
[L3 note_structurer](D9.4 NoteStructurerService 3 入口 — D9.6.1 经 ClipboardCaptureService 编排)
  ↓ minimax M3 → Markdown + tags
  ↓ INSERT/UPDATE notes
  ↓ is_private=True → PrivateSkipDecisionReport 业务硬阻断(不调 LLM,沿 D4.7.2 v1.0.6 SPAM 范本)
  ↓ DB 锁 → FailureDecisionReport(reason="db_failure") 透传(D9.6.1 严判)
[Mac 通知 + 菜单栏更新]
...
| 文档 | 用途 |
|------|------|
| [docs/architecture.md](architecture.md) | 5 层架构 + 关键决策 + 适配器契约 + 数据流示例(本文件)|
| [docs/week1-mvp.md](week1-mvp.md) | Week 1 计划(D1-D5:邮件 + 日程),D1-D5.7.2 全部锁定 |
| [docs/week2-mvp.md](week2-mvp.md) | Week 2 计划(D6-D10:财务 + 笔记),2026-06-07 编写 |
| [docs/v0.1-launch-plan.md](v0.1-launch-plan.md) | **v0.1 发布规划(D6+D7+D9+D10 4 子阶段 + 收口,2026-07 中下旬发布)** |

---

**最后更新**:2026-06-15(D9.6 5 修复 5 commits 收口链 `9ce1367 → 21b9b90 → abe3d96 → 4dd47a7 → 9afebfe` 实化;1858 passed / 5 skipped / coverage 89.5% / **9 质量门 9/9 全绿**(8 原 + 1 coverage fail_under=80);下一棒 D10 启动)
