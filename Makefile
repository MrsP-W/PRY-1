...
# P0-4/P1 launchd 健康脚本位于 scripts/，不纳入历史 CLI/spike 脚本范围；
# 因此在常规 Ruff、format 与 mypy 门中显式覆盖，避免质量漂移漏检。
LAUNCHD_HEALTH_SCRIPTS := scripts/sample_launchd_health.py scripts/monitor_launchd_health.py

.PHONY: mypy
mypy: ## 9 质量门 — mypy 类型检查(严格模式 `--strict`,沿 v0.2.42 范本:43 errors 清零 + 失败即阻塞)
	@echo "$(BLUE)🔍 mypy 类型检查(严格模式 --strict)$(RESET)"
	@$(PYTHON) -m mypy --strict src tests $(LAUNCHD_HEALTH_SCRIPTS)

.PHONY: ruff
ruff: ## 9 质量门 — ruff lint 检查
	@echo "$(BLUE)🔍 ruff lint 检查$(RESET)"
	@$(PYTHON) -m ruff check src tests $(LAUNCHD_HEALTH_SCRIPTS)

.PHONY: format
format: ## 9 质量门 — ruff format 检查(--check 模式)
	@echo "$(BLUE)📐 ruff format 检查(--check 模式)$(RESET)"
	@$(PYTHON) -m ruff format --check src tests $(LAUNCHD_HEALTH_SCRIPTS)

.PHONY: format-fix
format-fix: ## ruff format 自动修复
	@$(PYTHON) -m ruff format src tests $(LAUNCHD_HEALTH_SCRIPTS)
