...
| 测试 | pytest + 覆盖率 | D1.1 覆盖率 0% → 62% → D4.8 90.2% → D5.1-fix 91.1%(1385 passed)→ D5.5.3 1522 passed / 90.2% → D5.5.4 1532 passed / 90.1% → D5.5.5 1534 passed / 90.3% → D5.6.4 1561 passed / 90.4% → D5.6.5 1563 passed / 90.4%(真实 SMTP 1 封新增 2 集成测试)→ S6 e2e 1738 passed / 90.1% → D9.5+S7 e2e 1839 passed / 89.6% → **D9.6 5 修复 1858 passed / 89.5%(fail_under=80 硬门槛)** → D10.1-D10.4 **1955 passed / 1 skipped / 89.48%**(D10.5 收口)→ post-tag 修复 **1958 passed / 1 skipped / 89.48%** → v0.2 B1/B2 docs 收口 **1964 passed / 1 skipped / 89.5%**(B1.1 6 优先级 + B1.2 注释漂移同步 + B2.1 sla_due_at_ms + B2.2 SLA 临近排序) → v0.2 B4.1 recipient_blacklist 落子层 **1980 passed / 1 skipped / 89.45%**(alembic 0010 + Store/严判/16 tests) → v0.2 B4.2 OutboxAdapter 黑名单 hot-path 阻断 **1988 passed / 1 skipped / 89.46%**(store_and_emit 入口接 is_blocked + union return type + 4 isinstance 收窄 + 8 tests) → v0.2 B4.2 closure blacklist_store type 严判 **1991 passed / 1 skipped / 89.47%**(`_validate_outbox_blacklist_store` helper + type(...) is 严格 + 3 负例测试 dict/list/subclass 拒收) → **v0.2 B4.3 落地**(EmailSendAdapter SMTP 发送路径二次防御,send_and_emit 入口前再调 is_blocked,4 重防误发 + SMTP_REAL_NETWORK 门控沿 d5-6-5-real-send 范本)→ **B4 收口 docs 已完成**(4 docs-only 同步:launch-plan §B4 + week2-mvp §B4 + architecture 黑名单流程图 + substage-mapping §3 重写) |
...
**最后更新**：2026-06-16 晚（v0.1 收口后发布前修复 + v0.2 B 阶段启动状态:HEAD `cab322c`,`git tag v0.1.0` 仍在 commit `2af775f` 不动。D10.1 7 Agent 角色契约 53 tests + D10.2 monthly_report.py + 14 tests + D10.3 launchd_install.sh + 21 tests + D10.4 S8+S9 e2e 实化 9 tests + D10.5 release-notes 8 段 5 commits 收口链落地;post-tag 已补 Spike A/B/C/D 4 个真跑证据(Spike A 6/15 dry-run cleanup + Spike B 6/16 真跑 3 步 + Spike C 6/16 月报真实生成 + Spike D 6/16 launchd 真实部署 5 源判定全过),`sync_notes.py` 真跑安全阀、launchd install/uninstall/race/`--month` 字面修;v0.2 B 阶段已启动:B1.1 `OutboxPriority` 扩 3 → 6 类 + B1.2 注释漂移同步 + B2.1 `sla_due_at_ms` 字段 + alembic `0009_sla_due_at` + B2.2 Dispatcher SLA 临近排序 + B4.1 recipient_blacklist 落子层(alembic 0010 + Store/严判/16 tests) + B4.2 OutboxAdapter store_and_emit 入口 hot-path 接 is_blocked 阻断(union return type + 4 isinstance 收窄 + 8 tests) + B4.2 closure blacklist_store type 严判(`_validate_outbox_blacklist_store` helper + type(...) is 严格 + 3 负例测试 dict/list/subclass 拒收) + B4.3 EmailSendAdapter SMTP 发送路径二次防御(send_and_emit 入口前再调 is_blocked + 6 tests + union return type + record_send_business_blocked_and_emit(reason="blacklisted_recipient"),4 重防误发 + SMTP_REAL_NETWORK 门控沿 d5-6-5-real-send 范本,dispatcher.py union 收窄);当前 `make test` **1997 passed / 1 skipped / coverage 89.48%**,`mypy src tests` / `ruff check` / **`ruff format --check .`(扩)** / `alembic upgrade head --sql` / `uv build` / `make lint` / **`coverage fail_under=80`** 9 质量门 9/9 全绿。当前 9 端到端真实收口状态:**9/9 已实化**(S7 ⌥⌘N 触发降级但 insert 链路以 spike 30 笔 faker 代偿验证 + S8 月报真实生成(Spike C) + S9 launchd 真实部署(Spike D,装载实化,真触发等 6/23 全链路重启手动 kickstart 补足))。下一棒:D8 智能财务 LLM 异常交易检测评估 docs-only(commit `cab322c`,推荐方案 A 规则基础 + C 商家画像组合,不选 B LLM 因违反 CLAUDE.md 数据不出本机铁律)+ 跨项目 memory 沉淀(commit `6844b1c` Agent Assistant 配套)+ README 漂移修正 docs-only(本 commit)= **v0.2 6 子阶段 docs-only 评估全闭环** = B1/B2/B4 已实化 + B-5/outlook/gmail/D8 docs-only 评估完成(commits `b172dae` + `31f8ed2` + `cab322c` 整 3 docs-only commits)→ **不封口 v0.1.0**(用户 6/16 晚明确指令,`git tag v0.1.0` 锚定 `2af775f` 不动,6/23 不跑 kickstart + seal 脚本,也不打 v0.2.0 tag;具体实施留 v0.2.1+ 用户决策启动 = B-5 沿 Quartz 替换 pynput 双进程范本 + outlook/gmail 沿 provider 工厂 + OAuth 2.0 抽象层 + D8 沿 A+C 组合 = 估 11-16 commits / 4-5 工作日 / 依赖增量 +13MB)。）
**当前模型**：MiniMax-M3
**维护者**：Mr-PRY
