...
>
> **状态**:🎯 **v0.2.33 W3 真账单 `--max-rows 5` 小扩容验证已收口**(2026-06-24 · 状态固化 3 文件 + spike-5 跑通)。承接 v0.2.32 W3 真账单 spike + 撞坑 #49(`AlipayCSV2027RealParser` 处理真实字段格式)+ `detect_version` 扫前 30 行找真 header。**spike-5 结果**:`--max-rows 5` → `parsed=5 inserted=4 categorized=4 duplicates=1 needs_confirm=0 failed=0 candidate_count=0 version=2027` ✅;6 维度稳定性验证 ✅(`inserted(4) + duplicates(1) = parsed(5)` 公式成立);v0.2.29 导出复用 OK(导出 1 行 = v0.2.27 spike 残留,本次 spike 全 categorized);v0.2.31 汇总脚本复用正常。**撞坑 2 类本轮新增**:#50 状态文档漂移(v0.2.29 收口后 README/SESSION/MODIFICATION-LOG 状态停在 6/23,v0.2.30/31/32 三 commit 链未同步 → 范本:每个收口都应顺手同步 3 文件)+ #51 duplicates=1 链路逻辑(L1 source 内 UNIQUE 是预期行为,不是 bug,spike 链路复用同一 CSV 时 `inserted + duplicates = parsed` 公式必成立)。**v0.2.33 状态固化**:Phase 1 先修复 3 文件漂移(v0.2.29 → v0.2.32 顶部同步)→ Phase 2 跑 `--max-rows 5` spike → 撞坑 #50/#51 沉淀。9 commit 链(v0.2.25-v0.2.33)+ 撞坑 18 类沉淀(本轮新增 #50/#51)。验证:**2265 passed / 1 skipped / 0 mypy / 0 ruff / 0 MD lint**。HEAD `9e9754a`。**当前进入 v0.2.34 `--max-rows 10` 小扩容验证准备就绪**(不等全量 49 笔)。边界:不真发邮件、不 kickstart launchd、不移动 `v0.1.0` tag(`2af775f`)、不打 `v0.2.x` tag(8/1 锚定策略)。详见 [docs/v0.2.33-w3-spike-5-2026-06-24.md](docs/v0.2.33-w3-spike-5-2026-06-24.md)。
