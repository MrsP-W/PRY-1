...
>
> **当前状态（2026-07-21，优先于下方历史详述）**：P3 Day0=`2026-07-21T18:23:47Z` collecting（调度冻结）；v1.1 进化底座 docs-only（能力清单 + `tests/eval/`）。**3160 passed / 1 skipped / 90.26%** / MD lint 298。
>
...
│       └── menu_bar/         # Mac 菜单栏 UI
├── tests/                    # pytest 单元测试(以 `make test` 输出为准 · 当前 3160 passed / 1 skipped / 90.26% · fail_under=80 硬门槛)
├── docs/                     # 设计文档
...
```bash
make test    # pytest 单元测试(以 `make test` 输出为准 · 当前 3160 passed / 1 skipped / 90.26% · fail_under=80 硬门槛)
```
