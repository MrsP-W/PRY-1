...
>
> **状态**:🎯 **v0.2.35 W3 真账单 `--max-rows 25` 阶梯验证已收口**(2026-06-24 · 阶梯验证 1 → 5 → 10 → 25 全部跑通)。承接 v0.2.34 W3 真账单 `--max-rows 10` 小扩容验证已收口 + 撞坑 #50 第二层范本 + #52 阶梯验证范本 + #53 跨 spike 累计公式校验。**spike-25 结果**:`--max-rows 25` → `parsed=25 inserted=15 categorized=15 duplicates=10 needs_confirm=0 failed=0 candidate_count=0 version=2027` ✅;6 维度稳定性验证 ✅(`inserted(15) + duplicates(10) = parsed(25)` 公式成立);v0.2.29 导出复用 OK(导出 1 行 = v0.2.27 spike 残留,本次 spike-25 全 categorized 无新增 needs_confirm);v0.2.31 汇总脚本复用正常。**阶梯验证范本 1 → 5 → 10 → 25 4 阶段**:全部公式 `inserted + duplicates = parsed` 成立(1+0=1 / 4+1=5 / 5+5=10 / 15+10=25);`duplicates` 单调递增(0 → 1 → 5 → 10);`needs_confirm` / `candidate_count` 全程 0。**撞坑 #53(本轮新增) 跨 spike 累计公式校验**:全 spike 链路 Σ(inserted) + Σ(duplicates) = Σ(max-rows) = 41 成立(25 + 16 = 41 = 1+5+10+25)✅;这是"完整性证明" — 任何数据丢失或重复 bug 都会被公式击穿。**撞坑 #50 双层防御范本沿用**:第一层 = v0.2.34 启动前状态固化(已收口),第二层 = v0.2.35 spike 跑完后二次纠偏(本次)。12 commit 链(v0.2.25-v0.2.35 + v0.2.33 二次纠偏)+ 撞坑 20 类沉淀(本轮新增 #53)。验证:**2265 passed / 1 skipped / 0 mypy / 0 ruff / 0 MD lint**。HEAD 以 `git rev-parse --short HEAD` 为准(不写精确 hash,避免自引用漂移)。**下一步候选**:全量 49 笔 spike(需用户授权)/ `--max-rows 49`(同全量)/ P1-1 mypy tests 13 errors / outlook/gmail SMTP spike(等授权 + Keychain 凭据)。边界:不真发邮件、不 kickstart launchd、不移动 `v0.1.0` tag(`2af775f`)、不打 `v0.2.x` tag(8/1 锚定策略)。详见 [docs/v0.2.35-w3-spike-25-2026-06-24.md](docs/v0.2.35-w3-spike-25-2026-06-24.md)。
