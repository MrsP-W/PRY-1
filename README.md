...
>
> **状态**：🎯 **v0.1.0 正式发布前最后收官**(2026-06-16 上午)。D1-D7 + S6 e2e + D9.1-D9.6 Apple Notes 全链路 + S7 e2e + D10 Agent 集成 + 月报 + launchd + **6/16 A 阶段真跑 3 步按顺序**(Notes 1 + ⌥⌘N 降级 + SMTP 1)全部收口 + **6/16 上午 B 阶段真跑 2 步**(Spike C 月报真实生成 + Spike D launchd 真实部署)全部收口 + **6/16 下午 v0.2 B1/B2 docs 收口**(B1.1 6 优先级扩展 + B1.2 注释漂移同步 + B2.1 sla_due_at_ms + B2.2 SLA 临近排序) + **6/16 下午 v0.2 B4.1 收件人黑名单配置表落子层**(alembic 0010 + Store/严判/16 tests,B4.2/B4.3 待启动)。D5.7.2 业务调度器真正锁定(B3 真实 SMTP 解封);D6+D7 微信/支付宝跨源去重;D9 Apple Notes + ⌥⌘N(降级);D10 7 Agent 角色 + monthly_report.py + launchd_install.sh;**9 端到端 9/9 已实化**(S7 ⌥⌘N 触发链路降级 → 业务层以 spike 30 笔 faker 验证 insert 链路代偿 / S8 月报真实生成(Spike C 6/16) / S9 launchd 真实部署(Spike D 6/16,装载实化,真触发等 6/23 全链路重启时手动 kickstart 补足))。当前 `make test` **1980 passed / 1 skipped / coverage 89.45%**,**9 质量门 9/9 全绿**(8 原 + 1 fail_under)。**Git tag `v0.1.0`** 已落(commit `2af775f`,tag 不动);当前 HEAD `0c6472f` 为 post-tag docs 收口状态(v0.2 B4.1 落子层)。详见 [docs/v0.1-release-notes.md](docs/v0.1-release-notes.md) / [reports/v0.1-e2e-scenarios.md](reports/v0.1-e2e-scenarios.md) / [reports/v0.1-real-spike-b-real-2026-06-16.md](reports/v0.1-real-spike-b-real-2026-06-16.md) / [docs/architecture.md](docs/architecture.md)。
...
│       └── menu_bar/         # Mac 菜单栏 UI
├── tests/                    # pytest 单元测试(1980 passed / 1 skipped,覆盖率 89.45%,fail_under=80 硬门槛)
├── docs/                     # 设计文档
...
**最后更新**：2026-06-16 下午（v0.1 收口后发布前修复 + v0.2 B 阶段启动状态:HEAD `0c6472f`,`git tag v0.1.0` 仍在 commit `2af775f` 不动。D10.1 7 Agent 角色契约 53 tests + D10.2 monthly_report.py + 14 tests + D10.3 launchd_install.sh + 21 tests + D10.4 S8+S9 e2e 实化 9 tests + D10.5 release-notes 8 段 5 commits 收口链落地;post-tag 已补 Spike A/B/C/D 4 个真跑证据(Spike A 6/15 dry-run cleanup + Spike B 6/16 真跑 3 步 + Spike C 6/16 月报真实生成 + Spike D 6/16 launchd 真实部署 5 源判定全过),`sync_notes.py` 真跑安全阀、launchd install/uninstall/race/`--month` 字面修;v0.2 B 阶段已启动:B1.1 `OutboxPriority` 扩 3 → 6 类 + B1.2 注释漂移同步 + B2.1 `sla_due_at_ms` 字段 + alembic `0009_sla_due_at` + B2.2 Dispatcher SLA 临近排序;当前 `make test` **1980 passed / 1 skipped / coverage 89.45%**,`mypy src tests` / `ruff check` / **`ruff format --check .`(扩)** / `alembic upgrade head --sql` / `uv build` / `make lint` / **`coverage fail_under=80`** 9 质量门 9/9 全绿。当前 9 端到端真实收口状态:**9/9 已实化**(S7 ⌥⌘N 触发降级但 insert 链路以 spike 30 笔 faker 代偿验证 + S8 月报真实生成(Spike C) + S9 launchd 真实部署(Spike D,装载实化,真触发等 6/23 全链路重启手动 kickstart 补足))。下一棒:6/20 B4 blacklist_recipients(3 commits)/ B-5 pynput 评估(1 docs-only)→ 6/21 D8 智能财务 + outlook/gmail 评估(各 1 docs-only)→ 6/22 v0.2 docs 收口 + 9→11 端到端场景预映射 → 6/23 全链路重启时手动 `launchctl kickstart -k gui/$(id -u)/com.myaiemployee.agent` 1 次补足 S9 真触发 → 当天 v0.1.0 正式发布封口(选 C 方案,见 `Agent Assistant/memory/v0.1-launch-decision-option-c-2026-06-16.md`)。）
**当前模型**：MiniMax-M3
**维护者**：Mr-PRY
