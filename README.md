...
>
> **当前状态（2026-07-20，优先于下方历史详述）**：Dashboard 已具备 AI 每日情报台与 Codex 本地对话笔记；P0-4/5 已收口，P1 独立 15 分钟只读巡检已部署并通过 T+15 自动采样验收。P1.5 新闻刷新已部署且首轮成功（`RunAtLoad=true`、`StartInterval=3600`、`KeepAlive=false`；仅公开 HTTPS Feed → 本地原子缓存，不启停服务、不 SMTP）。**3109 passed / 1 skipped / 90.36%** / MD lint 293 / mypy 279 files；下一棒为 P2 数据链路幂等与坏数据隔离。
>
...
│       └── menu_bar/         # Mac 菜单栏 UI
├── tests/                    # pytest 单元测试(以 `make test` 输出为准 · 当前 3092 passed / 1 skipped / 90.34% · fail_under=80 硬门槛)
├── docs/                     # 设计文档
...
```bash
make test    # pytest 单元测试(以 `make test` 输出为准 · 当前 3092 passed / 1 skipped / 90.34% · fail_under=80 硬门槛)
```
