...
>
> **状态**:✅ **v0.2.52 SMTPProviderFactory 协议不匹配修复(撞坑 #61)+ Makefile alembic 退出码修复(撞坑 #62)+ 状态三入口同步**(2026-06-25 · 承接 v0.2.51 5 步连击)。v0.2.52 三件事完成:**① P0 撞坑 #61** EmailSendAdapter smtp_provider 模式误把 `SMTPConnector`(高层封装,`async connect()` 无 host/port/timeout)塞进 `_smtp_transport` → 运行时 `TypeError: connect() got unexpected keyword argument 'timeout'`。修复:取 `connector.transport`(底层 `SMTPTransport`)赋给 `_smtp_transport` + 暴露 `_provider_default_host/port/email` 3 字段供 OutboxDispatcher 参考。**② P2 撞坑 #62** Makefile `alembic upgrade head --sql 2>&1 | head -50` `head` 会吃 alembic 退出码导致失败时 `make alembic` 仍报 0。修复:先写 `/tmp/alembic_head.sql` 再 `head -50` 并 `exit $?` 透传退出码。**③ P1** SESSION-STATE/MODIFICATION-LOG 顶部漂移到 v0.2.51 → v0.2.52(README L7)。2 新测试覆盖(`test_smtp_provider_mode_assigns_underlying_transport_not_connector` + `test_send_and_emit_smtp_provider_factory_end_to_end`)。**9/9 质量门**:2268+2 = 2270 passed / 1 skipped / mypy --strict 0 errors / 209 files / ruff 全绿 / alembic SQL 完整 / MD lint 141 files 0 errors。HEAD 以 `git rev-parse --short HEAD` 为准。**下一步候选**:v0.2.52.1 OutboxDispatcher 自动路由(用 `_provider_default_host/port` + `_smtp_provider` 字段,需用户授权) / 8/1 v0.2.1 release tag 锚定复评 / 凭据 Keychain 就绪后真实 SMTP spike。**撞坑累计 62 类**(本轮新增 #61 协议不匹配 + #62 head 吃退出码)。**沿用边界**:不真发邮件、不写入真实凭据、不 kickstart launchd、不移动 `v0.1.0` tag(`2af775f`)、不打 `v0.2.x` tag。详见 [docs/v0.2.52-smtp-provider-factory-protocol-mismatch-2026-06-25.md](docs/v0.2.52-smtp-provider-factory-protocol-mismatch-2026-06-25.md)。
