...
>
> **状态**：🎯 **v0.2.26 W3 虚拟账单 spike 2345 行端到端报告已收口**(2026-06-23 · 报告锚 `c0a8af9` · 纯 spike + docs,0 src/tests 改动)。v0.2.25 P0 二修已修复真账单 `--max-rows` 透传与 launchd seal bash bad substitution,相关验证 42 passed + ruff/bash -n 通过。当前进入 **W3 真账单授权等待态**:等用户提供真实微信/支付宝 CSV 后,只允许 `--max-rows 1` 小样本导入验证。边界:不真发邮件、不真导入账单、不 kickstart launchd、不移动 `v0.1.0` tag(`2af775f`)、不打 `v0.2.0/v0.2.1/v0.2.25/v0.2.26` tag。详见 [docs/v0.2.26-w3-virtual-bill-spike-2026-06-23.md](docs/v0.2.26-w3-virtual-bill-spike-2026-06-23.md)。
