...
>
> **状态**:🎯 **v0.2.36 W3 真账单 `--max-rows 49` 全量入库已收口**(2026-06-24 · 选项 B 路径 · 阶梯 1 → 5 → 10 → 25 → 49 五阶段全部跑通)。承接 v0.2.35 W3 真账单 `--max-rows 25` 阶梯验证已收口 + 撞坑 #50 第二层范本 + #52 阶梯验证范本 + #53 跨 spike 累计公式校验 v2.0 + **#54(本轮新增) 选项 B 优于选项 A 范本**。**spike-49 结果**:`--max-rows 49` → `parsed=49 inserted=24 categorized=24 duplicates=25 needs_confirm=0 failed=0 candidate_count=0 version=2027` ✅;6 维度稳定性验证 ✅(`inserted(24) + duplicates(25) = parsed(49)` 公式成立);v0.2.29 导出复用 OK(导出 1 行 = v0.2.27 spike 残留,本次 spike-49 全 categorized 无新增 needs_confirm);v0.2.31 汇总脚本复用正常。**阶梯验证范本 1 → 5 → 10 → 25 → 49 5 阶段**:全部公式 `inserted + duplicates = parsed` 成立(1+0=1 / 4+1=5 / 5+5=10 / 15+10=25 / 24+25=49);`duplicates` 单调递增(0 → 1 → 5 → 10 → 25);`needs_confirm` / `candidate_count` 全程 0。**撞坑 #53 v2.0(本轮升级) 跨 spike 累计公式校验 + 全量入库**:`Σ(inserted) + Σ(duplicates) = Σ(max-rows) = 90` 成立(49 + 41 = 90 = 1+5+10+25+49)✅;**5 阶段累计公式 + 全量入库验证 = 业务链路完整性证明的最高级形式**。**撞坑 #54(本轮新增) 选项 B 优于选项 A 范本**:"带 max-rows 守护的全量入库" = 全量入库效果 + 边界自守可逆性;撞坑 #18 风险门控 + #50 双层防御 + #51 链路逻辑三范本交叉应用。**撞坑 #50 第二层范本沿用**:第一层 = v0.2.34 启动前状态固化(已收口),第二层 = v0.2.36 spike 跑完后二次纠偏(本次)。14 commit 链(v0.2.25-v0.2.36 + v0.2.33 二次纠偏 + v0.2.35 漂移小修)+ 撞坑 21 类沉淀(本轮新增 #54)。验证:**2265 passed / 1 skipped / 0 mypy / 0 ruff / 0 MD lint / 88.77% coverage**。HEAD 以 `git rev-parse --short HEAD` 为准(不写精确 hash,避免自引用漂移)。**下一步候选**:P1-1 mypy tests 13 errors / outlook/gmail SMTP spike(等授权 + Keychain 凭据)/ 7/1 月度复盘。边界:不真发邮件、不 kickstart launchd、不移动 `v0.1.0` tag(`2af775f`)、不打 `v0.2.x` tag(8/1 锚定策略)。详见 [docs/v0.2.36-w3-spike-49-2026-06-24.md](docs/v0.2.36-w3-spike-49-2026-06-24.md)。
