...
>
> **当前状态（2026-07-19，优先于下方历史详述）**：Dashboard 已具备 AI 每日情报台与 Codex 本地对话笔记；后者只接收显式 JSONL，默认校验、`--apply` 才写入，覆盖原子幂等、归档/加密/来源隔离保护。**3091 passed / 1 skipped / 90.34%** / MD lint 292 / mypy 277 files；P0-4 24h 稳定观察仍是下一棒。
>
...
│       └── menu_bar/         # Mac 菜单栏 UI
├── tests/                    # pytest 单元测试(以 `make test` 输出为准 · 当前 3091 passed / 1 skipped / 90.34% · fail_under=80 硬门槛)
├── docs/                     # 设计文档
...
```bash
make test    # pytest 单元测试(以 `make test` 输出为准 · 当前 3091 passed / 1 skipped / 90.34% · fail_under=80 硬门槛)
```
