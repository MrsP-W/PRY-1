...
>
> **状态**:🎯 **v0.2.34 W3 真账单 `--max-rows 10` 小扩容验证已收口**(2026-06-24 · 阶梯验证 1 → 5 → 10 跑通)。承接 v0.2.33 W3 真账单 `--max-rows 5` 小扩容验证已收口 + 撞坑 #50/#51 沉淀(状态文档漂移双层防御 + L1 UNIQUE 预期行为)。**spike-10 结果**:`--max-rows 10` → `parsed=10 inserted=5 categorized=5 duplicates=5 needs_confirm=0 failed=0 candidate_count=0 version=2027` ✅;6 维度稳定性验证 ✅(`inserted(5) + duplicates(5) = parsed(10)` 公式成立);v0.2.29 导出复用 OK(导出 1 行 = v0.2.27 spike 残留,本次 spike-10 全 categorized 无新增 needs_confirm);v0.2.31 汇总脚本复用正常(6 维度渲染正常,counterparty 麦当劳(朝阳店) 1 行)。**阶梯验证范本 1 → 5 → 10**:三个阶段公式 `inserted + duplicates = parsed` 全部成立;`duplicates` 单调递增(0 → 1 → 5 = 上一阶段 inserted 累加);`needs_confirm` / `candidate_count` 全程 0。**撞坑 #52(本轮新增)**:阶梯验证范本(1 → 5 → 10 三阶段公式校验,比"一次跑满"更能暴露链路问题)。**撞坑 #50 双层防御范本已落地**:第一层 = v0.2.33 启动前状态固化(已收口),第二层 = v0.2.34 spike 跑完后二次纠偏(本次)。11 commit 链(v0.2.25-v0.2.34 + v0.2.33 二次纠偏)+ 撞坑 19 类沉淀(本轮新增 #52)。验证:**2265 passed / 1 skipped / 0 mypy / 0 ruff / 0 MD lint**。HEAD `aa2a937` → 本轮 docs-only 收口。**下一步候选**:`--max-rows 25` 继续阶梯验证 / 全量 49 笔(需用户授权) / P1-1 mypy tests 13 errors / outlook/gmail SMTP spike(等授权 + Keychain 凭据)。边界:不真发邮件、不 kickstart launchd、不移动 `v0.1.0` tag(`2af775f`)、不打 `v0.2.x` tag(8/1 锚定策略)。详见 [docs/v0.2.34-w3-spike-10-2026-06-24.md](docs/v0.2.34-w3-spike-10-2026-06-24.md)。
