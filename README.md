...
>
> **状态**:🟢 **v0.2.53.8 Dashboard opt-in 真实 Notes + Expense**(2026-06-25 · 沿 `DASHBOARD_REAL_DB=1` 注入 `NoteConfirmServiceImpl` + `ExpenseServiceImpl` · 共享 session_factory · 单项失败降级 Stub · 默认零 I/O)。**质量门**:**2330 passed / 1 skipped / 88.46%** / mypy --strict 0 errors(219 files) / ruff 全绿 / format 233 files / **MD lint 157 files** 0 errors。**下一棒**:`/api/reports` 只读端点 / outlook+gmail Keychain → 真实 SMTP / 8/1 截点。**边界**:不真发邮件、不写凭据、不默认读取 Keychain 明文(需显式 env 门控)、不写 DB、不 kickstart launchd、不打 `v0.2.x` tag。
