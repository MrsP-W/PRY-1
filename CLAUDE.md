...
> **当前状态（2026-07-23 19:59Z，优先于下方历史）**：P3 新 Day0=`2026-07-23T19:59:04.517676Z`，`collecting` / `attention=[]`；首份日报门=`2026-07-25T00:00:00Z`。失败 epoch 已移动至 `burn-in-archive/epoch-2026-07-23T02-30-20Z/` 并保留 `p3-awake` 延长前后回滚证据；health/news 间隔、Dashboard 200、stderr 增量全绿。仅把临时 caffeinate 延长到 9 天，不改业务调度；保留原 6 个 P3 未提交文件和 UI 改动，本轮不 commit / SMTP / v1.0 / Feature Flag。
>
This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

> **我的AI员工** — 全天候个人 AI 数字员工（与 Agent Assistant 兄弟项目，2026-06-12 落地 L4 Agent 层软链）
>
> **当前状态（2026-07-28 03:42Z，优先于下方历史详述；已合入 ce975f5）**：P3 新 Day0=`2026-07-27T05:34:24Z` **`status=attention`** · `attention=["health_unhealthy_sample", "health_alert_opened"]`(旧 epoch `fail_attention` 已归档 + Step 2 Dashboard 死锁修复后 `/health` 200 OK ~2ms,141 样本中 10 个不可用历史不消除);v1.1 进化底座 docs-only + D6.10.1 评测样本扩 4→15。**撞坑 #106 二修**(PR #5 squash merge `63eb8bb` · 独立 worktree `/tmp/my-ai-employee-106-fix-v2` 修复 + 4 新回归测试 + `scripts/check_quality_snapshot.py` 3 处子进程全部 `sys.executable -m pytest` + 重复 `import sys` 唯一化 + `Makefile` `LAUNCHD_ONE_SHOT_SCRIPTS` 覆盖 `scripts/check_quality_snapshot.py`)。**3200 passed / 1 skipped / 90.29%** / **314 MD** / mypy **294 files**(撞坑 #107 P1#1 baseline drift 撤回 · Path B 保守 revert 3182→3179 · working tree 3 个 untracked `test_p3_rollover_epoch.py` tests 仍 collect 但不计入 baseline · 符合 commit 原则)。**Step 1-3 全环收口**:PR #4 squash merge `3c5f9ac` · PR #5 squash merge `63eb8bb` · Step 2 Dashboard 死锁修复 · Step 3 等首日报门 2026-07-29T00:00:00Z · 本地 main pull + 远程 `p0-minimal-fix` + `codex/pitfall-106-fix-v2` 分支已删。WIP 7 files 已 commit/push(3 commits `53c0df8` + `8dfaab8` + `1b1f8dc` + D-step plan 入库 `225c350`)+ 撞坑 #107 二修候选(untracked 独立 worktree `/tmp/my-ai-employee-rollover-fix-v2`:P1#2 gating 改 allow-list + P2#3 后置 try/except + watch_once kwarg bug 修 + 9 tests 全绿);本轮新增待 commit:`quality_snapshot.py` baseline 3182→3179 revert(撞坑 #107 P1#1 撤回)+ 5件套同步。1 untracked `docs/v1.1-p3-observation-d-step-plan.md`(已 push)。
>
> **当前状态（2026-07-30 12:30Z，优先于下方历史详述）**：**邮件按钮修复 `3e8fc70` 已合并 main**(commit `537cace` · cherry-pick onto `ab807bb` · 0 冲突自动合并 · `docs/ui/codex-style-dashboard.html` +19 行 CSS · `#mail-queue .queue-actions .btn.decide-btn:not(.reject)` 高优先级选择器)。Step 1 全链收口 ✅:worktree `/tmp/my-ai-employee-3e8fc70-merge` + cherry-pick + 9/9 质量门全绿(`3200 passed / 1 skipped / 90.29%` · mypy --strict 0 errors) + push origin main `ab807bb..537cace` + ff-only merge。`main = origin/main = 537cace` · 16 untracked WIP 维持(撞坑 #103 红线)· 不动 8 个 P3 epoch 文件(`scripts/p3_rollover_epoch.py` / `tests/scripts/test_p3_rollover_epoch.py` / `ops/claude-p3-watch*` / `plugins/p3-ops-claude/*`)· 不动旧 rollover worktree(`/tmp/my-ai-employee-rollover-fix-v2`)。**Step 2 一次性部署 + 重启 + 新闻刷新**等用户 `deploy` + `restart` + 「新闻刷新动作」单独授权(避免连续部署两次)· **Step 3** rollover 候选升级为 tracked `--dry-run` + 隐私脱敏 + 测试待 Step 2 后。**红线全维持**:不抢控制权 / 不联网外传 / 不收费 SaaS / SMTP 真发需授权 / Notes 真同步默认 dry-run / `ENABLE_PATH_4_WRITE=1` 不写 / `ENABLE_NOTES_ENCRYPTION=1` 不写 / v1.0 tag 不打 / git push 需 `push` 关键词 / launchctl load -w 数字员工需授权 / 不替 sudo / 不动 LaunchAgent load / health/news 调度 / docs-only 期间不动业务代码 / 不动 P3 epoch 治理文件 / 不删旧 worktree。
>
> 最后更新:2026-07-19(**Day 14 + 撞坑 #95 修复(`commit 74d1d65` · 拆 menu-bar + Dashboard 为 2 独立 LaunchAgent)✅ + P0-3 caffeinate 1h 观察 ✅(menu-bar + dashboard 持续 1h 1min 23s 零重启 + 127.0.0.1:8765 LISTEN) + 撞坑 #97 修复 ✅(SQLCipher 跨线程 close · `sqlcipher_compat.py` 长生命周期 db_path 改用 NullPool · 不 StaticPool · 2 回归测试 5 passed) + 撞坑 #98 NEW(launchd 升级场景旧 digital-employee 迁移缺口 · K1-K4 4 回归测试 + launchd_install.sh 5.5 legacy retirement 段) + v0.2.78 收口 + v0.2.74/v0.2.74.1/v0.2.75/v0.2.76/v0.2.77 收口** · outbox 2 条已 `cancelled` · 未 SMTP 外发 · P0 `send_one_approved` 先校验收件人再审批 · 9 质量门 **3074 passed / 1 skipped / 90.39%** / **293 MD** / mypy **273 files** · **T3 L4 撞坑 #94 B 路径修复实战完全通过**(数字员工 launcher exit 0 + 9/9 预检全过 + err.log 空 + menu_bar.log B 路径 DB 反复 open/close 模式) + **撞坑 #95 NEW**(`ProcessType=Background` 不允许 fork · menu_bar/dashboard 50s 内被 launchd 回收 · 数字员工 plist `state=not running` · 1h 观察窗启动中锚 agent+imap-sync)· **1h/24h 观察启动选项 D · A+B 混合**(0 改动 · 等 #95 修复 D-step 后再走 24h 完整数字员工)(当前 HEAD 以 `git rev-parse --short HEAD` 为准) · **v0.2.65 preflight + v0.2.66 P2 preflight + v0.2.67 P2 真 install + v0.2.68 P3 prep + v0.2.69 P3-C + 撞坑 #81 bootout 收口 docs-only + v0.2.73 deploy-only + v0.2.74 #92 修复 B + v0.2.74.1 #92 巡检报告 + v0.2.75 #93 修复 + v0.2.76 T3 L4 #93 实战验证 + v0.2.77 T3 L4 #94 B 路径实战验证 + 撞坑 #95 NEW 暴露**(3 wrapper + 3 plist 部署 · 4 wrapper 用绝对路径 /opt/homebrew/bin/uv + ops 脚本用 ${UV_BIN} 检测 · launchd 实际 2/3 active + 1/3 注册(数字员工 launcher 一次性 RunAtLoad 模式 · 进程已退)· 完成度:**整体 ~94%**/**可无人值守 ~82%**(↓ 6pp · #95 限制 menu_bar/dashboard 常驻)/**v1.0 ~87%**(↓ 2pp)) #92 修复 B path migration ✅ + 撞坑
...
**当前阶段（2026-07-27，优先于下方历史详述）**：P0–P2 收口；**P3 新 Day0 collecting**（`2026-07-27T05:34:24Z` · 旧 epoch `fail_attention` 已归档）;**v1.1 进化底座 docs-only + D6.10.1 评测样本扩 4→15**。9 质量门 **3200 passed / 1 skipped / 90.29%** / **314 MD** / mypy **294 files**(撞坑 #107 P1#1 baseline drift 撤回 · Path B 保守 revert 3182→3179 · working tree 仍 collect 3 个 untracked P3 tests 但不计入 baseline)。WIP 7 files 已 commit/push(3 commits `53c0df8` + `8dfaab8` + `1b1f8dc` + D-step plan 入库 `225c350`)+ 撞坑 #107 二修候选(独立 worktree 9 tests 全绿);1 untracked `docs/v1.1-p3-observation-d-step-plan.md`(已 push)。
...
**当前阶段**：**Day 13 阶段 2.3 + 撞坑 #86 + 撞坑 #81 实测命中 + 撞坑 #92 修复 B + 撞坑 #93 实战验证 T3 L4 完全通过 + 撞坑 #94 menu_bar DB context manager B 路径代码修复候选 + P0-P3 prep + P3-C dry-run 收口**(`process_inbox --execute --limit 10` 真跑 · outbox 2 条撞坑 #85 幻觉草稿已 `cancelled` · 未 SMTP 外发 · P0 审批顺序修复 · router 空 token 优雅降级已推送 · **T3 L4 数字员工 launchctl load -w 实战验证撞坑 #93 完全通过**(9/9 预检全过 + UV_BIN 链路 + err.log 无 Documents 错误 → bootout + 撞坑 #94 menu_bar SQLAlchemy pool RuntimeError 暴露 → db_path 模式代码修复候选已落地 → 数字员工 bootout 维持 · 待授权重启 1h/24h 观察)· **HEAD / origin/main 以 `git status --short --branch` + `git rev-parse --short HEAD` 为准** · **P1 质量门 3 门全绿(lint/check-snapshot/mypy)** · **P2 launchd 完整 install 已完成;T3 L2 wrapper 代码修复 + deploy-only runtime 刷新完成** · **撞坑 #91 修复完成(wrapper 改调 ~/bin runner;未自动 load · agent + imap-sync 2/3 job 仍注册 · 数字员工待授权实测)** · **撞坑 #92 修复 B 完成(runtime 路径全迁 ~/Library/Application Support/MyAIEmployee/ · T3 L3 load 实测撞坑 #92 验证 100% · 撞坑 #93 立即暴露)** · **撞坑 #93 修复完成(wrapper 绝对路径 /opt/homebrew/bin/uv + ops 脚本 ${UV_BIN} 检测 · 47/47 launchd 测试 PASSED · T3 L4 实战验证 100% 命中)** · **P3-C 失败审计 dry-run 已收口(0 SMTP/0 Notes 真同步 · 撞坑 #88 沉淀)** · **撞坑 #94 修复候选(menu_bar subprocess SQLAlchemy pool RuntimeError · make_sqlalchemy_engine 支持 db_path · 待真实 load 验证)** · 9/9 质量门 **3074 passed / 1 skipped / 90.39%** / **293 MD** / mypy **273 files** · **红线全维持**(撞坑 #1/#18/#59/#65/#71/#76/#78/#79/#81/#85/#86/#87/#88/#92/#93/#94/#97/#98 · 默认不启用 `ENABLE_NOTES_ENCRYPTION=1` / `ENABLE_PATH_4_WRITE=1` · 1-click SMTP 真发仍需每次临时授权 · P3 真实业务逐封命名收件人授权 · #94 待 launchd 实测 · launchd 数字员工不自动 load).

> **2026-07-11 状态纠正（优先于文首的历史“NEW/启动中”描述）**：#95 已完成 P0-3 1h 验证；#97 已用 `NullPool` 修复并补真实 HTTP 并发回归，#98 legacy retirement 已收口。只读核验发现 menu-bar 于 07:23 重启，P0-4 24h 零重启观察需重新计时。本自动化不 push、不打 tag、不做真实外部写入。
