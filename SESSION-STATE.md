...
**当前启动候选**:**`v0.2.1` tag 已落地(`71b4602` annotated · 撞坑 #60 反转)** + **`v0.2.1-rc1` tag 维持期作为历史快照沿用** + **Phase A+B+C 已收口(2026-07-01)** + **7 天计划 Day 1-5 连续收口** — 2620 passed / 88.95% / MD lint **240**。**下一棒**:Day 6 真实 CSV / Notes 真同步 / 一键启动包(候选) · 90 封 QQ SMTP spike 仍跳过。
...
| 核心质量门 | **2620 passed / 1 skipped** · **88.95%** coverage · mypy --strict 0 errors(**238 files**) · MD lint **240 files** 0 errors(以 `make test` / `make coverage` / `make lint` 实测为准 · `make check-snapshot` 防漂移) |
