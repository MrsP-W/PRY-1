...
**当前启动候选**:**v0.2.9 W3 真账单 spike docs-only 准备 + 撞坑恢复 3 步实战演练 3 已关闭(2026-06-22)**,**下一步进入 v0.2.10+ 候选决策** — 真实 SMTP 发送继续等用户授权 + 凭据 + B 类白名单决策；W3 真账单 spike 继续等用户真实 CSV；6/23 全链路重启。
...
| 6/22 | 周一 | **v0.2.9 W3 真账单 spike docs-only 准备 + 撞坑恢复 3 步实战演练 3(本轮)** — docs/v0.2.9-w3-real-bill-spike-prep-2026-06-22.md 新建 · 6 项启动条件 checklist + 4 重风险门控 + 3 个启动命令范本 + 5 阶段启动流程 · 撞坑恢复 3 步实战演练 3(沿 [[v0.2.4]] §3 机制 3) · 不真跑 spike · 等用户真实微信/支付宝 CSV | 🟢 |
| 6/23+ | 周二 | W3 真账单 spike 启动(沿 v0.2.9 §"启动流程 5 阶段"· 等真 CSV)+ outlook/gmail SMTP 真实发送 spike(沿 v0.2.7 §"启动流程 5 阶段")+ 全链路重启 | ⏸️ |
