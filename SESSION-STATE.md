...
**当前启动候选**:**v0.2.17 6/23 实操就绪最后冲刺 docs-only(5 阶段重验证实测就绪 + 6 类新撞坑沉淀)已关闭(2026-06-22)**,**下一步进入 v0.2.18+ 候选决策** — 6/23 周二全链路重启(跑 v0.2.13 §"7 阶段实战手册"+ v0.2.14 §"8/8 质量门 baseline 5/8 ✅ 实测就绪"+ v0.2.17 §"5 阶段重验证实测就绪");真实 SMTP 发送等用户授权 + 凭据 + B 类白名单决策；W3 真账单 spike 等用户真实 CSV。
