...
> **当前状态（2026-07-20，优先于下方历史详述）**：Dashboard AI 每日情报台与 Codex 本地对话笔记已落地。**P0-4/5 ✅** + **P1/P1.5 ✅** + **P2 坏数据隔离 ✅** + **P3 burn-in collecting**（Day0 `2026-07-20T19:04:33Z`）。**AgentRun 最小闭环已落地**：`runtime/` + `0018_agent_runs` + 邮件→草稿五步 + Trace/Eval + 本机 stdio MCP 白名单；**默认已接真 classifier/drafter**（可 stub 覆盖；SPAM/system sender 不调草稿；默认 dry-run 不 SMTP）。**[PR #3](https://github.com/MrsP-W/PRY-1/pull/3) 已合入 `main`**；重叠 Draft **[PR #2](https://github.com/MrsP-W/PRY-1/pull/2) 已关闭**。**3148 passed / 1 skipped / 90.26%** / lint **295** / mypy **291 files**。**下一棒**：最早 **2026-07-22T00:00Z** 后核验首份 UTC 日报；约 07-28 / 08-20 再判 7d/30d；不做 SMTP / Mac 重启 / v1.0。
>
...
**当前启动候选（2026-07-20）**：`v0.2.1` tag 已落地；P0–P2 ✅；P3 Day0 collecting；AgentRun 邮件→草稿已接真 classifier/drafter。**3148 passed / 1 skipped** / **90.26%** / MD lint **295** / mypy **291 files**；v1.0 默认不打。
...
| 分支 | `main` |
| 当前质量门（2026-07-20） | **3148 passed / 1 skipped** · **90.26%** coverage · mypy --strict 0 errors(**291 files**) · MD lint **295 files** 0 errors；AgentRun `email_to_draft` 真 AI 接线 + P3 Day0 `collecting`；Codex 对话笔记仅显式本地导入。 |
