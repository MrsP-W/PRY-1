...
**当前启动候选**:**`v0.2.1` tag 已落地(`71b4602`)** + **Day 10 收口 ✅** + Day 8-9 收口(沿用) — **2790 passed / 2 skipped** / 89.09% / MD lint **247** / mypy **248 files**。**下一棒**:本地 ahead commits 待 push(需用户授权) · 90 封 QQ SMTP spike 仍跳过。
...
| 核心质量门 | **2790 passed / 2 skipped** · **89.09%** coverage · mypy --strict 0 errors(**248 files**) · MD lint **247 files** 0 errors(以 `make test` / `make coverage` / `make lint` 实测为准 · `make check-snapshot` 防漂移) |
