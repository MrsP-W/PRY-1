...
**当前启动候选**:**`v0.2.1` tag 已落地(`71b4602`)** + **Day 14 + P3-A T3 L4 #95 修复(`74d1d65`) ✅ + P0-3 caffeinate 1h 验证 ✅ + P1-1 修 #97 NullPool + P1-2 修 #98 launchd legacy retirement + P1-4 K6 行为回归** — outbox 2 条已 `cancelled` · 未 SMTP 外发;launchd 实际 4/4 active(menu-bar + dashboard + agent + imap-sync)· **2939 passed / 1 skipped** / 89.10% / MD lint **291** / mypy **257 files**。**下一棒**:push 双 P1 修复 → P0-4 24h 完整观察窗(锚 stderr 无 #97 traceback · legacy 已 retire)→ P0-5 v1.0 tag 评估;v1.0 tag 默认不打。
...
| 核心质量门 | **2939 passed / 1 skipped** · **89.10%** coverage · mypy --strict 0 errors(**257 files**) · MD lint **291 files** 0 errors(以 `make test` / `make coverage` / `make lint` 实测为准 · `make check-snapshot` 防漂移 · v0.2.76 + pitfall-90/91/92/93/94/97/98 → 290) |
