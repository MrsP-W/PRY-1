...
| 核心质量门 | **3048 passed / 1 skipped** · **90.25%** coverage · mypy --strict 0 errors(**263 files**) · MD lint **292 files** 0 errors(以 `make test` / `make coverage` / `make lint` 实测为准 · `make check-snapshot` 防漂移 · NotesCipher v3 + v2 兼容) |
...
| 2026-07-18 NotesCipher L3 回归 | 加密 title 的 L3 ±1 天候选现先解密再归一化；认证失败密文跳过、旧明文兼容。`make test` **3048 passed / 1 skipped / 90.25%**；生产真加密仍关闭，P0-4 观察边界不变。 |
