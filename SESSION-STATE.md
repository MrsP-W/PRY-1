...
> **当前状态（2026-07-21，优先于下方历史详述）**：**P3 本轮长稳观察已中断**（Mac 约 `2026-07-21T05:45Z` 重启）→ `status=attention`，`attention=[health_sample_gap, news_run_gap]`。服务已恢复（`/health` ok），自愈正常 ≠ P3 通过。Day0=`2026-07-20T19:04:33Z` **不可覆盖**，证据保留；**7/22 首份日报不得记 PASS**（校验器已要求 `attention` 空）。AgentRun 真 classifier/drafter 已合入。**3153 passed / 1 skipped / 90.26%** / lint **295** / mypy **291 files**。**下一棒**：确认重启后下一轮 15 分钟巡检；是否 archive+新开 P3 epoch 待你拍板；不做 SMTP / 额外重启演练 / v1.0。
>
...
**当前启动候选（2026-07-21）**：`v0.2.1` tag 已落地；P0–P2 ✅；**P3 本轮中断**（`attention` gaps，7/22 不记 PASS）；AgentRun 真 classifier/drafter 已合入。**3153 passed / 1 skipped** / **90.26%** / MD lint **295** / mypy **291 files**；v1.0 默认不打。
...
| 分支 | `main` |
| 当前质量门（2026-07-21） | **3153 passed / 1 skipped** · **90.26%** coverage · mypy --strict 0 errors(**291 files**) · MD lint **295 files** 0 errors；P3 本轮 `attention` 中断（证据保留）；AgentRun 真 AI 接线仍在；Codex 对话笔记仅显式本地导入。 |
