...
**当前启动候选**:**v0.2.12 6/23 全链路重启实战前置 docs-only + dry-run 深化 + 撞坑恢复 3 步实战演练 6 已关闭(2026-06-20)**,**下一步进入 v0.2.13+ 候选决策** — 6/23 周二全链路重启(跑本实战前置 7 阶段);真实 SMTP 发送等用户授权 + 凭据 + B 类白名单决策；W3 真账单 spike 等用户真实 CSV。
