...
**当前启动候选**:**v0.2.8 release notes 收口 + v0.2.1 release tag 锚定策略同步 docs-only 已关闭(2026-06-22)**,**下一步进入 v0.2.9+ 候选决策** — 真实 SMTP 发送继续等用户授权 + 凭据 + B 类白名单决策；W3 真账单 spike 继续等用户真实 CSV；6/23 全链路重启。
...
| 6/22 | 周一 | **v0.2.8 release notes 收口 + v0.2.1 release tag 锚定策略同步 docs-only(本轮)** — docs/v0.2-release-notes-2026-06-22.md 新建 · 285 commits / 80 feat / 126 new tests / 2225 passed baseline / 8 大特性用户视角 + 8 项 tag 锚定前置条件 + B 类延后清单 5 项 7/1 评估方向 · 沿 D5.7.2 范本 8/1 锚定 · 不真发邮件 · 不打 v0.2.0/v0.2.1 tag | 🟢 |
| 6/23+ | 周二 | W3 真账单 spike(等真 CSV)+ outlook/gmail SMTP 真实发送 spike(沿 v0.2.7 §"启动流程 5 阶段")+ 全链路重启 | ⏸️ |
