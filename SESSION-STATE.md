...
> **状态**:✅ **v0.2.22 W3 真账单 faker dry-run 已收口**(无副作用实操:微信/支付宝 2022-2025 解析链路 4/5 通过 + fingerprint L2 3 对命中 + categorizer 9 商家归类 + merchants 654 条分布验证;2026 解析器 NotImplementedError 待真实样本补充)。当前 `v0.2.22+ / 6/23 实操授权候选`:阶段 1-5 + launchd + W3 dry-run 已 GO;阶段 6 W3 等真实 CSV;阶段 7 outlook/gmail SMTP 等用户授权 + Keychain 凭据 + B 类白名单决策。边界:不真发邮件、不真导入账单、不 kickstart launchd、不移动 `v0.1.0` tag(`2af775f`)、不打 `v0.2.0/v0.2.1` tag。
