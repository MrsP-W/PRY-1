...
**当前启动候选**:**v0.2.18 docs 假设错误类撞坑专项清单 + 撞坑恢复 3 步实战演练 11 docs-only(撞坑史 6 类首次专项固化 + 范本累计 11)已关闭(2026-06-22)**,**下一步进入 v0.2.19+ 候选决策** — 6/23 周二全链路重启(跑 v0.2.13 §"7 阶段实战手册"+ v0.2.14 §"8/8 质量门 baseline 5/8 ✅ 实测就绪"+ v0.2.17 §"5 阶段重验证实测就绪"+ v0.2.18 §"docs 假设错误类专项 5 校验");真实 SMTP 发送等用户授权 + 凭据 + B 类白名单决策；W3 真账单 spike 等用户真实 CSV。 — 6/23 周二全链路重启(跑 v0.2.13 §"7 阶段实战手册"+ v0.2.14 §"8/8 质量门 baseline 5/8 ✅ 实测就绪"+ v0.2.17 §"5 阶段重验证实测就绪");真实 SMTP 发送等用户授权 + 凭据 + B 类白名单决策；W3 真账单 spike 等用户真实 CSV。
