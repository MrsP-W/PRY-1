...
**当前启动候选**:**v0.2.13 6/23 全链路重启实战手册 docs-only + 撞坑恢复 3 步实战演练 7 已关闭(2026-06-20)**,**下一步进入 v0.2.14+ 候选决策** — 6/23 周二全链路重启(跑本实战手册 7 阶段);真实 SMTP 发送等用户授权 + 凭据 + B 类白名单决策；W3 真账单 spike 等用户真实 CSV。
...
| 6/23+ | 周二 | W3 真账单 spike 启动(沿 v0.2.9 §"启动流程 5 阶段"· 等真 CSV)+ outlook/gmail SMTP 真实发送 spike(沿 v0.2.7 §"启动流程 5 阶段")+ 全链路重启(沿 v0.2.10 + v0.2.11 + v0.2.12 + v0.2.13 §"7 阶段实战手册")| ⏸️ |
