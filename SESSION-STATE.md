...
> **最后更新**:2026-06-21 · **项目**:我的AI员工 · **当前 HEAD 以 `git rev-parse --short HEAD` 为准**
...
**当前启动候选**:**v0.2.7 outlook/gmail SMTP 真实发送 spike 准备 docs-only 已关闭(2026-06-21)**,**下一步进入 v0.2.8+ 候选决策** — 优先候选:6/22 周一 v0.2 release notes 收口 + v0.2.1 release tag 锚定策略同步；真实 SMTP 发送继续等用户授权 + 凭据 + B 类白名单决策；W3 真账单 spike 继续等用户真实 CSV。
...
| 6/21 | 周日 | **v0.2.7 outlook/gmail SMTP 真实发送 spike 准备 docs-only(本轮)** — docs/v0.2.7-...md 新建 · 6 项启动条件 checklist + 5 重风险门控 + 3 个启动命令范本 + 5 阶段启动流程 · 撞坑恢复 3 步实战演练 2(沿 [[v0.2.4]] §3 机制 3) · 不真发邮件 · 8/8 质量门 baseline 验证 | 🟢 |
| 6/22 | 周一 | v0.2 release notes 收口 + v0.2.1 release tag 锚定策略同步 | ⏸️ |
| 6/23+ | 周二 | W3 真账单 spike(等真 CSV)+ outlook/gmail SMTP 真实发送 spike(沿 v0.2.7 §"启动流程 5 阶段") | ⏸️ |
