...
**当前启动候选**:**`v0.2.1` tag 已落地(`71b4602`)** + **Day 13 + P3-A T1 dry-run ✅ + T3 L2 #91 deploy-only 安全部署 ✅(v0.2.73)** — outbox 2 条已 `cancelled` · 未 SMTP 外发;launchd 2/3 注册;数字员工 1/3 未自动 load(#91 wrapper/runner 已刷新,待授权实测) — **2913 passed / 1 skipped** / 89.10% / MD lint **278** / mypy **256 files**。**下一棒**:T3 L3(#91 真实 load 复验授权)→ P3-B 新草稿+命名收件人 SMTP → P4/P5;v1.0 tag 默认不打。
...
| 核心质量门 | **2913 passed / 1 skipped** · **89.10%** coverage · mypy --strict 0 errors(**256 files**) · MD lint **278 files** 0 errors(以 `make test` / `make coverage` / `make lint` 实测为准 · `make check-snapshot` 防漂移 · v0.2.73 + pitfall-90/91 → 278) |
