...
**当前启动候选（2026-07-19）**：`v0.2.1` tag 已落地；Dashboard AI 每日情报与 Codex 本地对话笔记均已落地，后者未读取桌面历史、未触发外部写入。**3091 passed / 1 skipped** / **90.34%** / MD lint **292** / mypy **277 files**。下一棒：P0-4 24h 完整观察窗 → P0-5 v1.0 tag 评估（默认不打）。

**当前启动候选**:**`v0.2.1` tag 已落地(`71b4602`)** + **Day 14 + P3-A T3 L4 #95 修复(`74d1d65`) ✅ + P0-3 caffeinate 1h 验证 ✅ + P1-1 修 #97 NullPool + P1-2 修 #98 launchd legacy retirement + P1-4 K6 + P1-5 K7 install 行为回归** — outbox 2 条已 `cancelled` · 未 SMTP 外发;4 个 plist 已部署，但 GUI launchd 域当前未注册（恢复需单独授权）· **3074 passed / 1 skipped** / 90.39% / MD lint **292** / mypy **273 files**。**下一棒**:P0-4 24h 完整观察窗(锚 stderr 无 #97 traceback · legacy 已 retire)→ P0-5 v1.0 tag 评估;v1.0 tag 默认不打。
...
| 分支 | `main` |
| 当前质量门（2026-07-19） | **3091 passed / 1 skipped** · **90.34%** coverage · mypy --strict 0 errors(**277 files**) · MD lint **292 files** 0 errors；Codex 对话笔记仅显式本地导入。 |
...
| 核心质量门 | **3074 passed / 1 skipped** · **90.39%** coverage · mypy --strict 0 errors(**273 files**) · MD lint **292 files** 0 errors(以 `make test` / `make coverage` / `make lint` 实测为准 · `make check-snapshot` 防漂移 · NotesCipher v3 + v2 兼容) |
