...
**当前启动候选**:**`v0.2.1` tag 已落地(`71b4602` annotated · 撞坑 #60 反转)** + **`v0.2.1-rc1` tag 维持期作为历史快照沿用** + **Phase A+B+C 已收口(2026-07-01)** — 2611 passed / 88.97% / MD lint **233**。**下一棒**:#3 outlook-gmail 真实凭据激活候选(用户单独决策反转)/ #4 9→11 e2e spike 候选 / 8/1+ 月度复盘候选 · 90 封 QQ SMTP spike 仍跳过。
...
| 核心质量门 | **2611 passed / 1 skipped** · **88.97%** coverage · mypy --strict 0 errors(**238 files**) · MD lint **233 files** 0 errors(以 `make test` / `make coverage` / `make lint` 实测为准 · `make check-snapshot` 防漂移) |
...
| 7/1(本日-Day 3 C 路径真发 1 封)| 周三 | **Day 3 C 路径真发 1 封收口(撞坑 #76/#78/#79 5 重门控全开 + 撞坑 #59 QQ 例外激活 + 撞坑 #81 已修复)**(用户决策点 C:SMTP 真发但不走 IMAP · 收件人=自己 477753009@qq.com · 5 重门控全 OK)· `SMTP_REAL_NETWORK=1 uv run python scripts/spike_send_100.py --real --recipient 477753009@qq.com --max-recipients 1 --count 1 --confirm yes-i-understand-this-sends-real-email --smtp-host smtp.qq.com --smtp-port 465 --smtp-username 477753009@qq.com --smtp-provider qq --batch-size 10` 一次跑通 · **1 封 SENT 成功**(outbox_id=1 · from=to=477753009@qq.com · host=smtp.qq.com:465 · 调度延迟 4.6s < 5000ms 撞坑 #18 红线)· Keychain 真读 16 字符授权码(撞坑 #1 教训维持 · 不打印内容)· 报告 `output/spike/spike_send_100_20260701_140144.md`(2.6KB · 6 项通过 + 1 项 REAL 模式不适用)· `ops/day3-c-real-send-1-closure.md` 新写(9 节 · 用户决策 + 实际命令 + 实测结果 + 撞坑累计更新 + outlook/gmail 红线维持 + 报告归档 + 9/9 质量门 baseline + Day 4 候选 + 维护者)+ 6 文件 MD count 同步(232 → 233 · 撞坑 #50 漂移防御)+ `make check-snapshot` + `make lint` 四重防御全绿 + commit(预计)· 业务代码 0 改动(撞坑 #71 沿用 · spike 模式临时 DB 不影响 baseline)· 撞坑 #59 outlook/gmail 红线维持(本次只走 `--smtp-provider qq` · 不构成 outlook/gmail 真实凭据激活)· 撞坑累计 81 类 0 新增 · 9/9 质量门 baseline 不变(2611 / 88.97% / 233 md / 238 mypy)· **Day 4 启动准备就位**:SMTP 真发链路已验证 + Keychain 真实授权码读取链路已验证 + OutboxDispatcher REAL 模式已验证 + 状态机全流程已验证 | ✅ |
