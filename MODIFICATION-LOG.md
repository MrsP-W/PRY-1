...
| **质量基线** | **3182 passed / 1 skipped** / **90.26%** / mypy --strict 0 / **292 files** / MD lint **305 files** 0 errors(含 untracked `test_p3_rollover_epoch.py` 3 tests 入 baseline · 以 `make test` / `make coverage` / `make lint` 实测为准 · `make check-snapshot` 防漂移 · v1.1 进化底座 docs-only + AgentRun + P3 + 撞坑 #95 修复回归 + 撞坑 #97 NullPool 接入 + 撞坑 #98 launchd legacy retirement 收口 + 撞坑 #102 cli too_early test 时序修复 + 撞坑 #103 stash-collected-drift 沉淀 + 撞坑 #104 docs-only MD 同步 + **D6.10.1 评测样本 4→15 + 撞坑 #105 NEW docs+fixture 复合漂移待沉淀 + 撞坑 #106 二修 3 子进程 sys.executable + 重复 import 唯一化 + Makefile ruff scripts/ scope + 4 新回归测试 + 撞坑 #106 baseline post-merge untracked 3 tests sync 3179→3182**) |
