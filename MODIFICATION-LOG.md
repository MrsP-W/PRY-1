...
| **质量基线** | **2945 passed / 1 skipped** / **89.10%** / mypy --strict 0 / **257 files** / MD lint **291 files** 0 errors(以 `make test` / `make coverage` / `make lint` 实测为准 · `make check-snapshot` 防漂移 · v0.2.76 + pitfall-90/91/92/93/94/97/98 + I1-I4(撞坑 #96) + J1-J4(撞坑 #95) + K1-K8(撞坑 #98 launchd legacy retirement) + 2 NullPool 回归测试(撞坑 #97) → 2945/1) |
...
| **P1-6 #98 uninstall 精确标签回归(本次)** | `test(launchd): cover exact label matching in uninstall`(2026-07-10 · 仅测试与当前状态快照 · 新增 K8 参数化 3 场景：精确 `com.myaiemployee.menu-bar` 必且仅触发 unload，正则近似与后缀近似 label 必不触发 unload/bootout；fake launchctl 与所有删除均限定临时 HOME，未触发真实 launchctl · 定向 70 passed / `bash -n scripts/launchd_install.sh` 通过 · 全量 `make test` 2945 passed / 1 skipped / 89.10%；make lint、mypy、ruff、alembic SQL、uv build 均通过 · README / SESSION-STATE / MODIFICATION-LOG / CLAUDE / launch-plan / quality snapshot 已同步；同时将当前入口中已过期的 #97 StaticPool 建议校正为实际 NullPool 修复 · 不执行 push、tag 或外部写入) |
