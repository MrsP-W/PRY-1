...
| **撞坑 #102 P1 ci too_early 时序修复(本次 commit TBD)** | `fix(test): #102 verify_p3_first_daily::test_cli_too_early 锚 today UTC midnight 动态 day0`(2026-07-27 · 业务代码改动=0 · docs-only + 1 test patch · **撞坑 #102 NEW**: `tests/scripts/test_verify_p3_first_daily.py::test_cli_too_early_exit_code` 用硬编码 `day0=2026-07-21T06:48:48Z`,已被 wall-clock 推进到 gate=`2026-07-23T00:00Z` 之后,固定写法不再 `too_early`,实跑固定退化 `assert 1 == 3`(exit 1 = `fail_attention` 而非 `too_early`)。**修复**:`day0 = datetime.now(UTC).replace(hour=0, minute=0, second=0, microsecond=0)`,today UTC midnight 作锚 → `gate = today.date()+2 days at 00:00Z` → 对 runner 的 wall-clock now 永是 future → exit 3 永真;`check_snapshot.py` 顺带 cascade:`test_collected_test_count_matches_snapshot_pytest` 在 fix 前因为 collected=3163 vs passed+skipped=3160+1 fail(差 2),fix 后 passed=3162 → collected 3163 与 pass+skip 3163 一致;`quality_snapshot.py` baseline 同步 3160/90.26/292/298 → 3162/90.27/292/298;5件套 README L9+77+120、CLAUDE L9+18、SESSION-STATE L6+20+35、MODIFICATION-LOG 本行+v0.2-launch-plan L264 全绿;`make ci` 9/9 全绿 3162 passed / 1 skipped / 90.27% / 298 MD / mypy 292 files。**红线全维持**：不动 P3 epoch 治理(待用户授权 归档 + 新 Day0 + pmset/caffeinate 策略)、不动 LaunchAgent/调度/SMTP/v1.0 tag、不 push。 |
...
- 新增 `news/` 本地链路：12 个受控 HTTPS Feed → 原子 JSON 缓存 → `GET /api/news/daily` → Dashboard；API 请求不访问外网。
- Dashboard 以“每日新闻”替换财务展示，提供全部、国内、国际、已核验原话、发言线索五类筛选，并每 5 分钟只重读本地缓存。
- 官方逐字引语仅接受 NVIDIA Newsroom 中白名单说话人、紧邻明确归因和逐字引号同时成立的条目；媒体报道只作发言线索。

**2. 风险点**

- 单源失败、全源失败或空解析均保留最后非空缓存并标记降级；Feed 重定向仅允许同 HTTPS origin、最多两跳。
- 当前没有自动生成国内大佬引语，也不抓取登录态社交媒体；P0-4 观察期内不安装或重载任何 LaunchAgent。
...
- `tests/db/test_notes_encryption_store.py` 新增固定认证 v2 title 向量的 L3 ±1 天回归：直接落库的 `enc:v2:` 标题先解密、再完成模糊标题归一化与候选关联。
- 覆盖 `NoteStore._find_l3_fuzzy_in_session()` 的独立解密路径，避免未来仅普通 `get/list` 保持兼容、L3 去重却漏读历史笔记。

**2. 风险点**

- 不修改生产代码、密文格式、Keychain、数据库或外部服务；生产真加密与 SMTP 均仍关闭。
- 认证失败的 v2/v3 密文继续被 L3 跳过，不能降级为标题或候选。

**3. 当前项目整体总结**

- 定向核心 + NoteStore 测试 **52 passed**；全量 `make test` **3049 passed / 1 skipped / 90.24%**。
- 下一棒：P0-4 24h 观察仍保持只读；Dashboard HTML 用户 WIP 继续不混入提交。

### 2026-07-18 [NotesCipher AES-GCM v3 写入与 v2 兼容] — 收口

**1. 本次修改内容**

- `NotesCipherImpl` 新写入改为 `enc:v3:` AES-GCM：16-byte salt 派生字段 AES-256 key、12-byte nonce，AAD 严格绑定字段名。
- 保留认证通过的 `enc:v2:` XOR+HMAC 只读解密；`enc:v1:`、未知版本、篡改、错字段和错密钥继续 fail-closed。未做数据库迁移或读取时自动重写。
- `cryptography>=48.0.0` 升为直接运行依赖；同步 NoteStore、Dashboard、菜单栏、Keychain 名称与 Day 10 dry-run 的 v3 断言，并新增固定 v2 向量的核心及真实库行读取回归。

**2. 风险点**

- 生产 `ENABLE_NOTES_ENCRYPTION=1` 仍默认关闭；本轮未读取 Keychain、未触碰生产 Notes/数据库、未进行外部写入。
- v2 为历史只读兼容格式，不应在新代码中继续写入；若未来启用生产加密，应按现有 opt-in 与单条恢复边界执行。
- 开局已有 `docs/ui/codex-style-dashboard.html` WIP 保持隔离，未纳入本轮提交。

**3. 当前项目整体总结**

- 定向 Notes 回归 **133 passed**，dry-run 退出 0，确认临时库仅新写 `enc:v3:` 且 UI 无 `enc:` 泄露。
- 全量基线：**3048 passed / 1 skipped / 90.25%**；Markdown lint、Ruff/format、严格 mypy、Alembic SQL、`uv build` 与 `make check-snapshot` 均通过。
- 下一棒：维持 P0-4 只读观察；不启用真实 Notes 加密或 SMTP。
...
- `NoteStore._find_l3_fuzzy_in_session()` 现在先以 `title` 字段配置解密库内标量，
  再执行 L3 标题归一化；认证失败或不可用密文返回 `None` 时直接跳过，避免密文参与候选匹配。
- `tests/db/test_notes_encryption_store.py` 新增 3 条回归：真实 `enc:v2:` 标题的 ±1 天 L3 命中、
  篡改密文 fail-closed，以及 Impl cipher 对直接落库旧明文标题的 L3 兼容。
- `quality_snapshot.py` 与当前状态入口按实测同步为
  **3035 passed / 1 skipped / 90.24% / 292 MD / mypy 263 files**。

**2. 风险点**

- 不改密文格式、迁移或 Keychain；`ENABLE_NOTES_ENCRYPTION=1` 仍默认关闭，XOR→AEAD 仍是独立 P1。
- 开局已有 `docs/ui/codex-style-dashboard.html` WIP 保持隔离，不纳入本轮提交。

**3. 验证证据**

- `make test`：**3035 passed / 1 skipped / 90.24%**；定向加密与 L3 链路 **34 passed**。
- Ruff/format、严格 mypy、Alembic SQL、`uv build`、Markdown lint 与 `make check-snapshot` 均通过。
...
- **fix(snapshot)**: `check_quality_snapshot.py` 在 pytest 子进程中跳过 guardian probe，并向 live pytest 传递 probe 标记，杜绝 `make check-snapshot` 的嵌套 pytest 高扇出。
- **test(launchd)**: 补回 K8 的 3 个 uninstall 精确标签场景；近似或后缀 label 不得触发 unload/bootout。
- **docs(state)**: 当前入口基线同步为 **2946 passed / 1 skipped**；实测 coverage **89.12%**，按 <0.1pp 抖动规则保持快照 **89.10%**。

**2. 风险点**

- 未执行真实 launchctl、push、tag 或任何外部写入；启动前既有 `docs/ui/codex-style-dashboard.html` 改动仍独立保留。
- 后续运行 `make check-snapshot` 应避免与另一份完整 pytest 并发，以免不必要的资源竞争。

**3. 当前项目整体总结**

- 质量门：`make test` **2946 passed / 1 skipped**（89.12%）；`MYAI_EMPLOYEE_SNAPSHOT_SKIP_LIVE_PYTEST=1 make check-snapshot`、`make lint`、mypy、alembic SQL、ruff、`uv build` 全绿。完整 `make check-snapshot` 的重复全量 pytest 在终端环境被 SIGTERM 中断，未见断言失败。
- 下一棒：保持本地提交边界；P0-4 的 24h 观察不在本自动化中自动启动。
...
## 67. 2026-07-03 · 快照校准 2806/2→2812/1 + check-snapshot 分布校验 + Day1.4 只读验收

> **触发**:`make test` 实测 2809/1 vs snapshot 2806/2 · collected 总数校验拦不住 passed/skipped 分布漂移

### 1. 本次修改内容

- **基线校准** — pytest `2806/2 → 2812/1 skipped`(含 +3 新测) · 五入口 + `quality_snapshot.py` 同步
- **check-snapshot 补强** — `count_live_pytest_outcomes()` 校验实跑 passed/skipped 分布 · 子进程测用 `MYAI_EMPLOYEE_SNAPSHOT_SKIP_LIVE_PYTEST=1`
- **Day 1.4** — `scripts/verify_day14_db_tables.py` + `make setup-verify-db`(transactions/notes/outbox schema+索引+行数,只读)
- **测试** — `test_live_pytest_outcomes_match_snapshot_when_mocked` · `test_verify_day14_db_tables` ×2 · setup_wizard `--tcc-only` 改进程内 `main()` 断言

### 2. 风险点

- 🟡 **check-snapshot 耗时** — `make check-snapshot` 默认多跑一轮全量 pytest(~2min);CI 已先跑 `make test` 可接受
- 🟢 **Day 1.3 未启动** — 真实 IMAP/账单/Notes 入库仍待用户授权

### 3. 当前项目整体总结

- 进度:**2812 passed / 1 skipped / 89.07%** / MD lint **256** / `make check-snapshot` 全绿(SKIP live 模式验证)
- 下一棒:Day 1.3 真实数据入库(用户授权后)

---

## 66. 2026-07-03 · Day 11 尾巴校准(memory 入库 + pytest/MD 口径)

> **触发**:Day 11 closure 后 `memory/day11-snapshot-guardian-drift-2026-07-04.md` 未跟踪 · snapshot 2790/2 vs `make test` 2791/1 显示漂移

### 1. 本次修改内容

- **memory 入库** — `memory/day11-snapshot-guardian-drift-2026-07-04.md`(撞坑 #50 第三层范本)
- **基线校准** — pytest `2790/2 → 2791/1 skipped` · MD lint `251 → 252 files` · coverage/mypy 不变
- **入口同步** — quality_snapshot + 五入口 + `reports/Day11-closure.md` §3/§7

### 2. 风险点

- 🟢 **撞坑 #50** — 稳态 collected(2792)==passed+skipped(2792);guardian fail 消失后显示口径须与 `make test` 一致

### 3. 当前项目整体总结

- 进度:**2791 passed / 1 skipped / 89.09%** / MD lint **252** / `make check-snapshot` 全绿
- 下一棒:本地 ahead 待 push(需用户授权)

---

## 65. 2026-07-03 · MD lint 快照校准 249→250

> **触发**:`make check-snapshot` 失败 · `git ls-files '*.md'` 250 vs snapshot 249 · pytest 维持 2790/2 不动

### 1. 本次修改内容

- **基线校准** — `quality_snapshot.py` MD lint `249 → 250 files` · pytest/coverage/mypy 不变
- **入口同步** — README / CLAUDE / SESSION-STATE / MODIFICATION-LOG / launch-plan 当前块对齐

### 2. 风险点

- 🟢 **撞坑 #50** — guardian fail 导致的 pytest expected+2 差值随 MD 修复消失;稳态 collected(2792)==passed+skipped(2792)

### 3. 当前项目整体总结

- 进度:**2790 passed / 2 skipped / 89.09%** / MD lint **250** / `make check-snapshot` 全绿
- 下一棒:本地 ahead 待 push(需用户授权)

---

## 64. 2026-07-02 · Day 10 check-snapshot P0 校准(2788→2790 / 246→247 MD)

> **触发**:Day 10 closure 后 `make check-snapshot` 失败 · MD lint 246→247 · pytest 2788→2790(撞坑 #50 guardian 3 fail 掩盖)

### 1. 本次修改内容

- **基线校准** — `quality_snapshot.py`:pytest `2788 → 2790 passed / 2 skipped` · MD lint `246 → 247 files` · coverage 维持 `89.09%`(以 `make coverage` 实测)
- **状态同步** — README / CLAUDE / SESSION-STATE / MODIFICATION-LOG / `docs/v0.2-launch-plan.md` 当前入口块对齐 snapshot
- **根因** — Day 10 Phase 4 closure commit 误将 passed 写低 2( guardian 测试自身 fail 时 under-report);`reports/Day10-closure.md` 新增 1 MD → lint 247

### 2. 风险点

- 🟢 **撞坑 #50 第四层防御** — `make check-snapshot` + `check_state_entries` 双重校验;稳态 collected(2792) == passed+skipped(2792)
- 🟢 **红线维持** — `ENABLE_NOTES_ENCRYPTION=1` / `ENABLE_PATH_4_WRITE=1` 不写 shell profile

### 3. 当前项目整体总结

- 进度:**2790 passed / 2 skipped / 89.09%** / mypy **248 files** / MD lint **247 files** / `make check-snapshot` 全绿
- 下一棒:本地 ahead commits 待 push(需用户授权)
...
## 61. 2026-07-04 · Day 11 收口(Phase 1-4 docs-only + 9 门全绿 + 撞坑 #50 第三层沉淀)

> **触发**:用户授权「按照推荐执行」= Phase 0 push 可选 + Phase 1 9 门复核 + Phase 2 docs 补丁 + Phase 3 三角色 + Phase 4 commit(今日 4 commits 已推 + Day 11 docs-only 计划)

### 1. 本次修改内容

- **Day 11 Phase 1** — 9/9 质量门全量复核:`make test` 2791 passed / 1 skipped · coverage 89.09% · mypy 0/248 · ruff check + format · alembic --sql 0016 · uv build · MD lint 250/0 · check-snapshot OK
- **Day 11 Phase 2.1** — 修补 `docs/day11-notes-encryption-production-runbook.md` §1.5:`247 MD` → `250 MD`
- **Day 11 Phase 2.2** — 写 `reports/Day11-closure.md` 8 章节(范围 / commit 链 / 9 门实测 / 8 端点矩阵 / Notes runbook 5 道门 / 撞坑沿用 / 风险红线 / Day 12+ 候选)
- **Day 11 Phase 2.3** — `@教练员` 沉淀:`memory/day11-snapshot-guardian-drift-2026-07-04.md` 撞坑 #50 第三层(MD + pytest 联动 7 步同步范本)
- **Day 11 Phase 3** — D-step 三角色:`@检查员` 9 门复核 / `@教练员` 撞坑 #50 第三层 / `@回顾员` Day11-closure 8 章节
- **Day 11 Phase 4(snapshot 校准)** — 7 步同步范本:apply `quality_snapshot.py + CLAUDE.md + README.md + SESSION-STATE.md + MODIFICATION-LOG.md + launch-plan.md + reports/Day10-closure.md`(本次 7 文件再校准 250→251 MD)
- **业务代码 0 改动** — Day 11 累计 0 改动(docs-only)

### 2. 风险点

- 🟢 **`ENABLE_NOTES_ENCRYPTION=1` 仍不写 shell profile**(runbook docs-only · §2 五步未执行)
- 🟢 **`ENABLE_PATH_4_WRITE=1` 仍 8/1 前不开**(companion readiness docs-only · §4 八步未执行)
- 🟢 **撞坑 #1 隐私铁律维持** — Keychain 凭据不入 docs / 不入 chat / 不入 commit
- 🟢 **撞坑 #18 严判沿用** — 5 门替代 ENABLE_PATH_4_WRITE
- 🟢 **撞坑 #50 第三层沉淀** — docs-only 阶段新增 MD 必须 7 步同步,否则 guardian 报 ERROR
- 🟢 **撞坑 #59 outlook/gmail 红线维持** — 不配置 SMTP
- 🟢 **撞坑 #64/#65/#71/#76/#78/#79 沿用** — 公共 API 一致性 / opt-in 4 阶段 / 业务代码改动解除 / 5 重门控 / spike thread-safe / redactor 严判
- 🟢 **挂起 4 项** — push(本地 ahead 4)/ Notes 真加密生产启用(等授权)/ Path4 8/1 实写(等授权)/ 90 封 SMTP(等授权)

### 3. 当前项目整体总结

- 进度:**2790 passed / 2 skipped / 89.09%** / mypy --strict **0 / 248 files** / MD lint **251 files** / `make check-snapshot` 全绿
...
- 🟢 **撞坑 #50 第三/四层同步范本二次激活** — 文档 MD +1 → 检查 scripts/check_quality_snapshot.py(L3)+ scripts/check_state_entries.py(L4)双层联动严判,7 步同步范本验证有效
- 🟢 **`ENABLE_NOTES_ENCRYPTION=1` 仍不写 shell profile** — 抽测仅跑 dry-run spike(临时 DB),生产主库未触碰
- 🟢 **`ENABLE_PATH_4_WRITE=1` 8/1 前不开** — companion 30 tests 仅验证只读契约,POST dry-run 默认仍未解锁
...
## 74. 2026-07-08 · 项目检查后 push 状态口径修正(docs-only)

> **触发**:项目检查确认 `HEAD = origin/main = ae071f0`,但 `SESSION-STATE.md` / 本日志仍残留"push 待授权"旧口径;本轮只做 docs-only 状态收口,不改业务代码,不触发真实发送。
...
- 🟢 **docs-only** — 未改业务代码、未改数据库、未真发 SMTP。
- 🟢 **不复用 #85 草稿** — 2 条 cancelled 草稿维持作废,后续真发必须从新草稿开始。
- 🟡 **历史条目保留当时语境** — 旧阶段的"待 push"记录只在最新状态处纠偏,不重写历史过程。
...
## 78. 2026-07-08 · 项目检查并优化 — v0.2.65 本地 ahead 状态口径收口(docs-only)

> **触发**:用户要求"检查项目并优化";实时检查发现本地已有 `f7c0b2f` preflight 收口提交,但远端仍停在 `75e2ec9`,且 `MODIFICATION-LOG` 顶部与 v0.2.65 文档局部仍容易误读为"当前已与远端同步"。

### 1. 本次修改内容

- **当前快照优化**:`MODIFICATION-LOG.md` 顶部改为"远端已到 `75e2ec9`,后续本地 ahead docs-only 待 push",避免把本地 `f7c0b2f` 误判为已推送。
- **preflight 文档优化**:`docs/v0.2.65-day13-preflight-hour-1-7-2026-07-08.md` 将 Hour 1 的 `HEAD = origin/main = 75e2ec9` 改成"基线复核锚 + 本地 ahead 待 push"双口径,并同步 MD lint 259。
- **验证闭环**:`make check-snapshot` / `make lint` / `check_state_entries` 均已通过。

### 2. 风险点

- 🟢 **docs-only** — 未改业务代码、未触发 SMTP、未写真实 DB、未 kickstart launchd、未打 tag。
- 🟢 **不默推** — 本地 ahead 继续等待用户明确 "push" 指令。
- 🟡 **远端状态仍旧** — `origin/main` 尚未包含 `f7c0b2f` 与本次优化提交,对外协作时以本地 `git log` 为准。
...
- **进度数字**:**2904 passed / 1 skipped** / **89.12%** / mypy **256 files / 0 errors** / MD lint **263 files / 0 errors** / `make ci` 9 门全绿沿用(含 `check-snapshot` 防漂移双门 OK · 撞坑 #50)。
- **撞坑累计**:**86 类 · 0 新增**(本轮 P3 prep docs-only 不引入新撞坑)。
- **当前阶段**:P0 已 push · P1 质量门 3 门全绿 · P2 launchd 真 install + 撞坑 #81 bootout 收口(`e10c995` 已 push)· **P3 启动准备 docs-only 已就位并推送**(`2eec62e = HEAD = origin/main`)· **P3 真实业务最小闭环执行 + P4 24h + P5 v1.0 tag 评估 仍需用户逐项授权**。
- **下一步**:
...
## 81. 2026-07-08 · 项目检查 — pytest outcome 快照漂移修复(2900/3 → 2904/1)

> **触发**:用户要求"项目检查";实时 `make check-snapshot` 失败:`quality_snapshot claims 2900 passed / 3 skipped, live pytest reports 2904 passed / 1 skipped`。本轮只修状态事实源与当前入口,不改业务逻辑。
...
- 🟢 **docs/snapshot-only** — 未改业务代码、未触发 SMTP、未写真实 DB、未 kickstart launchd、未打 tag。
- 🟢 **漂移根因明确** — collect 总数相同但 passed/skipped 分布变了,需以 live pytest outcome 为准。
- 🟡 **P3/P4/P5 仍待授权** — Notes 真同步、SMTP 单封真发、24h 观察、v1.0 tag 评估均不随本轮自动执行。
...
## 87. 2026-07-09 · P3-A T3 L2 撞坑 #91 wrapper 修复收口

> **触发**:用户选择 A,授权 T3 L2 / #91 修复。边界:不自动 `launchctl load -w`,不触发 SMTP,不做 Notes 生产同步,不启用 Path4,不打 v1.0 tag。

### 1. 本次修改内容

- **launchd wrapper 修复**:`scripts/launchd_install.sh` 新增 `~/bin/my-ai-employee-digital-runner`;`my-ai-employee-start` 改为设置 `MY_AI_EMPLOYEE_PROJECT_ROOT` 后调用 runner,不再 `exec bash <PROJECT_ROOT>/ops/start-digital-employee.sh`。
- **runner 根路径修复**:`ops/start-digital-employee.sh` 支持 `MY_AI_EMPLOYEE_PROJECT_ROOT`,复制到 `~/bin` 后仍定位真实项目根目录。
- **契约测试**:`tests/scripts/test_launchd_install.py` 新增 F4-F6,锁住 runner 部署、禁止 Documents ops exec、显式项目根路径 override。
- **审计文档**:新增 `docs/v0.2.72-p3-a-t3-l2-91-fix-2026-07-09.md`。

### 2. 风险点

- 🟢 **未自动 load**:本轮只改安装产物模板和测试,没有执行 `launchctl load -w`。
- 🟢 **0 真实业务**:未 SMTP 真发,未 Notes 生产同步,未 Path4 写入,未打 v1.0 tag。
- 🟡 **仍需真实验证**:下一棒需用户单独授权重新生成 `~/bin` wrapper/runner 并只加载数字员工 plist,确认 err log 不再出现 #91。
...
### 2026-07-20 [P1.5 AI 新闻 hourly one-shot] — 收口

#### 1. 本次修改内容

- **feat(ops)**：新增独立 `com.myaiemployee.news-refresh` LaunchAgent 与 wrapper；`RunAtLoad=true`、`StartInterval=3600`、`KeepAlive=false`、`ProcessType=Background`。安装/卸载链完整覆盖该 Job，常规 mypy/Ruff/format 显式覆盖真实 CLI 入口。
- **test(ops)**：新增无网络 CLI 回归，覆盖成功 JSON、全源失败保留旧缓存与并发刷新跳过；补强 install/uninstall 与 deploy-only 契约，确保新闻 one-shot 不含服务控制。
- **deploy**：先运行 `bash scripts/launchd_install.sh deploy-only`，再仅 bootstrap 新闻 Job；未 reload menu-bar、Dashboard、agent、IMAP 或 health-monitor。

#### 2. 运行验收与边界

- **首轮**：`news-refresh` 的 `runs=1`、`last exit code=0`、`run interval=3600 seconds`；首次从无缓存写入 48 条，stdout 显示 12 个公开来源均完成，stderr 为 0 字节。
- **Dashboard**：`GET /api/news/daily` 返回 `refresh_state=fresh`；缓存采用既有跨进程锁、原子替换与失败保留，不会因单源失败阻塞。
- **边界**：只发起白名单 HTTPS GET；不读账号/Keychain、IMAP 或业务日志正文，不调用 LLM，不发 SMTP，不重启或控制任何业务服务。`3600` 为约每小时，不承诺整点。

#### 3. 当前项目整体总结

- **质量证据**：定向新闻/launchd/快照回归 **89 passed**；`make test` **3109 passed / 1 skipped / 90.36%**；mypy **279 files**、Ruff、format、Markdown lint **293 files**、`make check-snapshot`、Alembic SQL 与 `uv build` 均通过。
- **下一棒**：P2 数据链路幂等与坏数据隔离；随后 P3 7 天 burn-in / 30 天无 P0/P1 与自动周报。v1.0 默认不打。

---

### 2026-07-20 [P2 数据链路幂等与坏数据隔离] — 收口

#### 1. 本次修改内容

- **fix(imap)**：`IMAPConnector.fetch` 对单封 envelope 解析加 `try/except`；坏 UID 记 `skipped_bad` 日志，兄弟邮件仍返回。
- **docs(sync)**：澄清 commit 粒度是「单批」非「单封」；envelope 隔离在 connector 层。
- **test**：+4 回归 — 坏 envelope 保兄弟 UID、坏 XML `ValueError`、单源坏 XML 仍写 snapshot、坏缓存 item 跳过不炸 payload。
- **边界**：Notes/Codex 既有 thread 幂等与 JSONL fail-closed 维持，本轮不改生产写入路径。

#### 2. 风险点

- IMAP 坏 envelope 仅跳过解析失败 UID，不掩盖整次 `fetch`/`search` 网络失败（仍走 circuit breaker）。
- 新闻单源 error 仍可能使 coverage 下降；全源失败继续保留旧缓存。
- 未触碰 SMTP、Path4、Notes 加密、真实 IMAP sync 或 launchd load。

#### 3. 当前项目整体总结

- **质量证据**：`make test` **3113 passed / 1 skipped / 90.39%**；mypy **279 files**、MD lint **293**；三入口 + `quality_snapshot` 已同步。
- **下一棒**：P3 7d/30d burn-in；v1.0 默认不打；Mac 重启/注销恢复另需授权。

---

### 2026-07-20 [P3 长稳 burn-in Day0] — 已启动

- **完整性修复**：Day0 marker 改用原子独占创建；健康与新闻 journal 均做完整语义/交叉字段校验，坏行、坏结构或不可读输入会阻止误判 PASS。
- **运行证据**：Day0=`2026-07-20T19:04:33.499091Z`；首条 health 为 `healthy=true`，首条新闻回执为 12/12 公开来源成功、48 条；P3 report=`collecting`、`attention=[]`。
- **自动报告**：先以 deploy-only 落盘，再只 bootstrap `com.myaiemployee.burn-in-report`；RunAtLoad `runs=1`、`last exit code=0`、stderr 0 字节，未 reload 既有服务。
- **质量证据**：`make ci` 9 门全绿：**3133 passed / 1 skipped / 90.41%**、mypy **280 files**、MD lint **293 files**、Alembic SQL、build 均通过。
- **边界/下一棒**：不做 SMTP、Mac 重启/注销或 v1.0 tag；首个完整 UTC 日后核验日报；每日 02:10 本地报告下，约 7/28 自动核验 7 天无人干预资格，约 8/20 才可判 30 天 PASS。

---

### 2026-07-20 [AgentRun 最小闭环] — 收口

#### 1. 本次修改内容

- **设计**：[`docs/superpowers/specs/2026-07-20-agent-run-minimal-loop-design.md`](docs/superpowers/specs/2026-07-20-agent-run-minimal-loop-design.md)
- **runtime**：`AgentRun` 表 `0018_agent_runs` + 状态机 + `email_to_draft` 五步（plan→classify→draft→await_approval→finalize）；默认 dry-run、默认不 SMTP
- **Trace/Eval**：Router 可选 `trace_id` + `last_trace()`；`scripts/eval_agent_runs.py` + fixtures
- **MCP**：`StdioTransport` 绝对路径白名单 + `GatedToolCaller` JSON Schema/只读/危险工具审批；Mock 仍为默认
- **lab**：`output/2026-07-20/labs/pydantic-ai-spike/README.md`（不迁移主工程）

#### 2. 风险点

- 邮件工作流当前用可注入 stub 分类/草稿，生产接真 classifier/drafter 需另开接线任务
- stdio MCP 未接远程 OAuth；危险工具必须显式 approve
- 不触碰 P3 Day0、SMTP 真发、Mac 重启、v1.0 tag
...
### 2026-07-20 [AgentRun 真 classifier/drafter 接线] — 收口

#### 1. 本次修改内容

- `email_to_draft` 默认经 `make_classify_fn` / `make_draft_fn` 接真 `EmailClassifier` / `EmailDrafter`
- SPAM / system sender / `SpamBlockedError` → `draft.blocked`，不写 outbox
- CLI `--stub-ai`；单测显式 stub；mock router 回归 +3 tests
- 仍默认 dry-run、不 SMTP

#### 2. 风险点

- 无 API key 时默认 live 路径会失败并 CHECKPOINTED（可 `--stub-ai`）
- 不重置 P3 Day0；不真发邮件

#### 3. 当前项目整体总结

- **质量证据**：`make test` **3155 passed / 1 skipped / 90.26%**
- **下一棒**：2026-07-22T00:00Z 后核验首份 UTC 日报

---

### 2026-07-20 [P3 首份日报核验脚本] — 收口

#### 1. 本次修改内容

- 新增 `scripts/verify_p3_first_daily.py`：最早 `2026-07-22T00:00Z` 才跑 report；校验 Day0 前缀 + `daily_written>=1`
- 未到窗 fail-closed exit 3；`--force` 仅调试
- +3 回归测试

#### 2. 风险点

- 不重置 Day0；不 SMTP；生产核验勿用 `--force`

#### 3. 当前项目整体总结

- 当前 UTC 仍 premature：`too_early` 已实测
- **质量证据**：`3155 passed / 1 skipped / 90.26%`（以 make test 为准）
...
- **质量证据**：3155 passed / 1 skipped / 90.26%

---

### 2026-07-21 [P3 重启中断 + 首份日报校验收紧] — 收口

#### 1. 本次修改内容

- 事实：Mac 约 `2026-07-21T05:45Z` 重启 → P3 `attention=[health_sample_gap, news_run_gap]`
- `verify_p3_first_daily`：有 `attention` 时 `result=fail_attention` / `ok=false`（防 7/22 误 PASS）
- 本机证据：`burn-in/epoch-interrupted-2026-07-21.json`；Day0 未重置

#### 2. 风险点

- 当前 epoch 不能证明连续无人值守；新开 epoch 需显式授权（Day0 不可覆盖）
- 不做 SMTP / 额外重启演练 / v1.0

#### 3. 当前项目整体总结

- 服务已恢复；**3155 passed / 1 skipped / 90.26%**
- **下一棒**：下一轮 15 分钟巡检确认 → 你决定是否新开 P3 epoch

---

### 2026-07-21 [P3 归档旧 epoch + 新开 Day0] — 收口

#### 1. 本次修改内容

- 旧 epoch（Day0=`2026-07-20T19:04:33Z`）归档至 `burn-in/archive/epoch-...`（永不记 PASS）
- 新 Day0=`2026-07-21T06:48:48Z`；`report`=`collecting` / `attention=[]`
- `verify_p3_first_daily` 改为按当前 epoch 动态门槛（现为 **2026-07-23T00:00Z**）；有 attention 仍 fail-closed

#### 2. 风险点

- 旧 journal 保留但不计入新 epoch；不 SMTP / 不额外重启 / 不 v1.0
- 7d/30d 计时相对新 Day0 重新起算

#### 3. 当前项目整体总结

- **3155 passed / 1 skipped / 90.26%**
- **下一棒**：新 epoch 长稳；约 07-28 / 08-20（相对新 Day0）再判资格

---

### 2026-07-21 [P3 间隔失准根因修复] — 进行中

#### 1. 本次修改内容

- 根因：`ProcessType=Background` 节流（health ~50min / news ~3h）+ 系统 sleep=1 冻结 StartInterval
- 修复：health/news → `ProcessType=Standard`；health `StartInterval=600`；去掉 Nice
- 新增 `scripts/verify_launchd_intervals.py`；deploy-only + bootout/bootstrap 已落地
- README 旧 3092/90.34% 漂移已清

#### 2. 风险点

- 系统睡眠仍会冻定时器；长稳需醒机或 caffeinate
- **暂不归档/新开第三 epoch**，等受控间隔验证通过

#### 3. 当前项目整体总结

- 验证窗约 12 分钟进行中；通过后再谈新开 P3

---

### 2026-07-21 [P3 间隔修复后新开 Day0] — 收口

#### 1. 本次修改内容

- 受控验证：health 连续 gap **10.0 / 10.0 min**（限 18）
- 归档失败 epoch `2026-07-21T06:48:48Z`；新 Day0=`2026-07-21T18:23:47Z` → collecting / attention=[]
- README 旧 3092/90.34% 已清理

#### 2. 风险点

- `pmset sleep=1` 仍会在睡眠时冻结 StartInterval；P3 需醒机或 caffeinate
- 新闻小时级验证未在短窗双采样；继续观察下一小时回执

#### 3. 当前项目整体总结

- HEAD 以 `git rev-parse` 为准；首份日报门槛 2026-07-23T00:00Z

---

### 2026-07-21 [v1.1 持续进化底座 docs-only] — 收口

#### 1. 本次修改内容

- 新增 `docs/v1.1-evolution-foundation.md`：能力注册表盘点、Feature Flag/反馈/SLO/知识包/Labs 分期门禁
- 新增 `tests/eval/`：SCHEMA + 4 条 synthetic 样本 + 契约测试（+5）
- 三入口 + quality_snapshot 同步：**3160 / 1 / 90.26% / 298 MD / mypy 292**；**零** LaunchAgent / 调度改动

#### 2. 风险点

- 评测集仅 4 条占位，距 30+ 目标仍差；需脱敏真实样本，禁止入库凭据
- Feature Flag / 反馈 runner 未实现，须等 P3 7d 通过后再动生产代码
...
### 2026-08-03 [D6.13.3 SLO 契约交叉审查修复] — 复验通过

#### 1. 本次修改内容

- 基于候选 `e8a8150` 和 FAIL 审查 `872f712`，只修原任务 3 个 docs 文件
- safety 改为 policy decision coverage，合法 deny 计入覆盖；latency 补 p99 breach
- freshness 纳入窗口两端区间；告警 schema 统一为三态 evaluation_status + resolved(pass)
- 任务验收绑定 `566c1a8...HEAD` 与精确三文件白名单，状态回到 `review`

#### 2. 风险点

- 5 个 SLO 仍全部 `inactive`；本修复不等于已监测、已告警或已获得发布资格
- P3 资格与运行时边界不变；本轮只恢复 docs 候选的 `ready_to_merge`

#### 3. 当前项目整体总结

- 修复仅 docs-only；未修改业务代码、LaunchAgent、SMTP、Feature Flag、`ENABLE_*` 或真实数据
- ref-bound diff、精确三文件白名单、Markdown/YAML/语义断言全绿；不合并、不 push
---
...
- 质量基线按 tracked 实跑对齐：**3360 passed / 1 skipped**（修正旧 3200 漂移 + 本批 +6）
...
### 2026-08-13 [授权 rollover] 新 Day0 已开 + watch_once 参数修复

#### 1. 本次修改内容

- 用户授权后：§5.1 备份 `burn-in.backup-2026-08-13T08-42-00Z`（27 文件 md5 一致）
- 执行 `python3 scripts/p3_rollover_epoch.py`：旧 epoch 已归档，新 Day0=`2026-08-13T08:42:06.833689Z`，`collecting` / `attention=[]`
- CLI 当时因 `watch_once(app_support_dir=...)` TypeError exit 1；FS 已成功，未二次执行、未回滚
- 修复调用并补回归测试；执行报告 `tests/eval/audit/p3-rollover-execution-2026-08-13.md`

#### 2. 风险点

- 未改 LaunchAgent / SMTP / Feature Flag；v1.1-A 仍 NOT_UNLOCKED
- 回滚路径：archive `epoch-2026-07-30T07-04-45Z` 或上述 backup
