...
| 维度 | 状态 |
|------|------|
| **当前阶段** | ✅ `v0.2.29` 交易候选 review/export 机制已关闭(`TransactionStore.list_by_needs_confirm` + `scripts/export_transaction_candidates.py` JSONL/CSV · 承接 v0.2.28 剩余 `candidate_count=367` 需人工 review · 只读导出,不导入/不改库/不自动合并 · 36 passed + ruff 0 + CLI help ok);当前进入 W3 真账单授权等待态(等用户真实微信/支付宝 CSV,只跑 `--max-rows 1`)|
...
| **质量基线** | v0.2.29 聚焦验证:36 passed / ruff 0 / CLI help ok；v0.2.28 全量质量门:2240 passed / 1 skipped / 88.77% coverage / mypy src+tests 0 / ruff 0 / alembic sql 0 / build ok / MD lint 0 |
