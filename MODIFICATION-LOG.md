...
| **质量基线** | **3164 passed / 1 skipped** / **90.26%** / mypy --strict 0 / **292 files** / MD lint **298 files** 0 errors(以 `make test` / `make coverage` / `make lint` 实测为准 · `make check-snapshot` 防漂移 · v1.1 进化底座 docs-only + AgentRun + P3 + 撞坑 #95 修复回归 + 撞坑 #97 NullPool 接入 + 撞坑 #98 launchd legacy retirement 收口 + 撞坑 #102 cli too_early test 时序修复 + 撞坑 #103 stash-collected-drift 沉淀) |
