...
> **生成时间**:20260618_212756
> **范围**:5 封入库 + 批量 APPROVED + OutboxDispatcher 循环 + 状态机推进 + SLA + 退避
...
- **入库总时长**:0.00s
- **outbox_id 范围**:1..5
- **D5.6.1 P1.3 批量审批 PENDING_SEND → APPROVED**:5/5
...
| 2 | InMemorySmtpTransport.sent_log 行数 == sent | 5 | 5 | ✅ |
...
- **5 封入库**:✅ (5/5)
- **批量审批 PENDING_SEND → APPROVED**:✅ (5/5)
- **OutboxDispatcher 循环 run_once**:✅ (1 轮)
- **状态机全部最终态**:✅
- **SLA 评估**:skip_breach=0
- **退避回路**:total_processed=5
