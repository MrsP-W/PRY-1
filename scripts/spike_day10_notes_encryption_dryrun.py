...
目的(用户 2026-07-02 路径 A Phase 3.5 授权):
    - 端到端验证:mock Keychain master key + 临时 SQLite DB + `ENABLE_NOTES_ENCRYPTION=1`
      → NoteStore insert Impl cipher → 库内认证 `enc:v3:` 前缀
...
4 退出码契约(沿 `scripts/spike_d8_1000.py` D8 spike 范本):
    0 = 成功(spike 跑通 + 统计输出)
    1 = 准备失败(env / 路径错)
    2 = 业务失败(库内未发现 `enc:v3:` 前缀 或 UI 仍返回密文)
...
def _seed_encrypted_note(
    ...
    *,
    ...
    """NoteStore.insert 走 Impl cipher(库内落认证 `enc:v3:` 前缀)."""
    result = store.insert(
        apple_note_id=apple_note_id,
        folder="Notes",
        title=title,
    ...
    try:
        ...
        try:
            ...
            encrypted_id = _seed_encrypted_note(
                ...
                title="新加密标题",
                body="新加密正文,应自动落 enc:v3: 前缀",
            )

            # ---- 4. 库内直查:验证 legacy 是明文,encrypted 是 enc:v3: 前缀 ----
            ...
            if is_notes_ciphertext(legacy_row.title) or is_notes_ciphertext(legacy_row.body):
                print(
                    f"[FAIL] legacy note 不应该有 enc: 前缀: title={legacy_row.title[:20]!r}",
                    file=sys.stderr,
                )
                return 2
            if not encrypted_row.title.startswith(
                _CIPHERTEXT_PREFIX_V3
            ) or not encrypted_row.body.startswith(_CIPHERTEXT_PREFIX_V3):
                print(
                    f"[FAIL] 新加密 note 应该有 enc:v3: 前缀: title={encrypted_row.title[:20]!r}",
                    file=sys.stderr,
                )
                return 2
            print("[OK] 库内前缀严判: legacy=plaintext, encrypted=enc:v3: 前缀")
            ...
            if (
                ...
                or encrypted_loaded.body != "新加密正文,应自动落 enc:v3: 前缀"
            ):
            ...
            if encrypted_by_id is None or encrypted_by_id.title != "新加密标题":
                ...
                return 2
            # 严判无任何 enc: 前缀泄露
            if is_notes_ciphertext(legacy_loaded.title) or is_notes_ciphertext(
                encrypted_loaded.title
            ):
                print(
                    f"[FAIL] 解密后仍含 enc: 前缀: legacy={legacy_loaded.title[:20]!r}, "
            ...
            print(
                ...
                "(legacy 明文 + encrypted 解密,无 enc: 前缀泄露)"
            )
            ...
            print("  notes: 2 (1 legacy plaintext + 1 new AES-GCM enc:v3:)")
