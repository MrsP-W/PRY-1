...
NEW_MERCHANTS: tuple[str, ...] = (
    "全家便利店", "苹果App Store", "京东购物", "海底捞火锅", "物美超市",
    "7-Eleven便利店", "兼职收入", "肯德基(海淀店)", "便利店", "奶茶店",
    "烧烤店", "早餐店", "地铁出行", "小吃店", "西贝莜面村",
    "外婆家", "南京大牌档", "喜茶", "奈雪的茶", "一点点",
    "蜜雪冰城", "益禾堂", "大润发", "永辉超市", "华润万家",
    "山姆会员店", "盒马鲜生", "叮咚买菜", "美团买菜", "京东到家",
    "朴朴超市", "嘀嗒出行", "高德打车", "T3出行", "曹操出行",
    "京东快递", "顺丰速运", "中通快递", "圆通速递", "韵达快递",
    "国家电网", "中国石化", "中国石油", "物业费", "房租",
    "喜马拉雅FM", "网易云音乐", "优酷会员", "爱奇艺会员", "腾讯视频",
)
...
def _generate_transactions(rng: random.Random, n: int = 1000) -> list[Transaction]:
    ...
    LONG_TAIL_MERCHANTS: tuple[str, ...] = (
        # 餐饮冷启动(15 个)
        "潮汕牛肉火锅", "南京盐水鸭", "广州早茶", "北京烤鸭店", "重庆小面",
        "兰州拉面", "沙县小吃", "黄焖鸡米饭", "螺蛳粉", "麻辣烫",
        "烧烤摊", "麻辣香锅", "酸菜鱼", "小龙虾", "海鲜大排档",
        # 购物冷启动(15 个)
        "无印良品", "宜家家居", "屈臣氏", "丝芙兰", "迪卡侬",
        "小米之家", "华为体验店", "苹果零售店", "vivo体验店", "oppo体验店",
        "国美电器", "苏宁易购", "百思买", "Costco", "麦德龙",
        # 服务冷启动(20 个)
        "美甲店", "理发店", "宠物医院", "牙科诊所", "眼镜店",
        "健身房月卡", "瑜伽课程", "游泳馆", "羽毛球馆", "网球场",
        "电影院", "KTV", "剧本杀", "密室逃脱", "保龄球馆",
        "美容SPA", "足疗按摩", "推拿理疗", "中医诊所", "心理咨询",
        # 其他冷启动(20 个)
        "银行手续费", "信用卡年费", "保险费", "股票买入", "基金定投",
        "理财产品", "P2P借款", "借呗还款", "花呗还款", "白条还款",
        "学费分期", "培训费", "考试报名", "签证费", "护照办理",
        "公证费", "律师咨询", "会计代账", "税务代办", "工商注册",
    )

    # 中频商家 23 个(各 ~15 笔 = 345 笔,这些商家历史积累到画像阈值)
    MID_FREQ_MERCHANTS: tuple[str, ...] = (
        "肯德基(海淀店)", "海底捞火锅", "西贝莜面村", "外婆家",
        "喜茶", "奈雪的茶", "一点点", "蜜雪冰城", "益禾堂",
        "大润发", "永辉超市", "华润万家", "山姆会员店", "盒马鲜生",
        "美团买菜", "京东到家", "朴朴超市",
        "嘀嗒出行", "高德打车", "T3出行", "曹操出行",
        "国家电网", "中国石化",
    )
