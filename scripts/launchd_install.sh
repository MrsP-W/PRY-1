...
# P1:5 job label(月报 / IMAP 同步 / 菜单栏 / Dashboard / 只读健康巡检)
...
if [[ "${MODE}" == "uninstall" ]]; then
    echo "===== uninstall 流程 ====="
    # 1. launchctl unload(6 label — P1 health monitor + legacy digital-employee)
    ...
    # 2. 删 plist(6 label · 含 health monitor / legacy digital-employee)
    for label in \
        com.myaiemployee.agent \
        com.myaiemployee.imap-sync \
        com.myaiemployee.menu-bar \
    ...
    # 3. 删 ~/bin/ 脚本(6 wrapper — 含 health monitor + legacy start)
    for script in \
    ...
    # 5. 验证无残留(6 label · 含 health monitor / legacy digital-employee)
    launchctl list > "${LC_OUT_UNINSTALL}" 2>&1 || true
    for label in \
        com.myaiemployee.agent \
        com.myaiemployee.imap-sync \
...
# ===== 4. 复制 plist(替换 $USER 占位符 · P1:5 job) =====
for src_plist in \
    "${SOURCE_PLIST}" \
    "${SOURCE_PLIST_IMAP}" \
    "${SOURCE_PLIST_MENUBAR}" \
    "${SOURCE_PLIST_DASHBOARD}" \
    "${SOURCE_PLIST_HEALTH_MONITOR}"; do
...
if [[ "${DEPLOY_ONLY}" == "true" ]]; then
    echo ""
    echo "🎉 deploy-only 完成(未调用 launchctl load -w · P1:5 job 部署 · 撞坑 #98 P1-3 修复:不退役 legacy)"
...
# ===== 6. launchctl load(5 job · P1:health monitor 独立 one-shot) =====
...
for target_plist in \
    ...
    "${TARGET_PLIST_HEALTH_MONITOR}"; do
...
# ===== 7. 5 源验证(5 job · P1 health monitor) =====
echo ""
echo "===== 5 源验证 ====="
...
for i in 1 2 3 4 5; do
    ...
    if [[ "${missing}" -eq 0 ]]; then
        LAUNCHCTL_OK=1
        echo "✅ 源 4(launchctl list): 5 job 已注册(retry ${i}/5)"
        break
    fi
done
if [[ "${LAUNCHCTL_OK}" -eq 0 ]]; then
    echo "❌ 源 4: launchctl list 未见全部 5 job(retry 5 次)" >&2
    cat "${LC_OUT}" >&2 || true
    exit 3
fi
...
echo ""
echo "🎉 launchd 部署完成(5 job · 含 P1 health monitor)!"
