...
def _write_report(result: SpikeResult, report_dir: Path) -> Path:
    ...
    lines.append("")
    lines.append("## 5. 脚本外质量门复跑记录")
    lines.append("")
    lines.append("- 本脚本仅验证 D7 虚拟 spike 5 段链路。")
    lines.append("- `make test` / `ruff` / `mypy` / `make lint` 以当次命令输出为准。")
    lines.append("- 若要固化质量门结果,请在复跑后手工更新本节,避免脚本写入过期结论。")
    lines.append("")
    lines.append("## 6. 结论")
    lines.append("")
