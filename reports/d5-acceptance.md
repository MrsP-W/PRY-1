# D5 业务调度器 — 验收报告(D5.6.1 模拟发送 spike 收口,D5.1-D5.5 全部锁定)

> **状态**:⏸️ **D5 业务调度器 v1.0 阶段收口**(D5.1 `cce567a` → D5.5.5 `a866810` + `caf021f` + D5.6.1 修复 `pending`)
...
> **2026-06-11 晚间 D5 启动 · 2026-06-13 D5.6.1 修复收口 · 维护者:Mr-PRY · 模型:MiniMax-M3**
>
> **D5.6.1 关键修正**:
> - **D5.6 v1 收口被检查员驳回**(2026-06-13):5 项缺陷(P0 凭证硬编码 / P1.1 措辞虚假 / P1.2 防误发缺失 / P1.3 绕过审批 / P2 失败断言恒真)
> - **B3(接真实 SMTP)撤回解封**:D5.6.1 spike 默认 InMemory 模拟,不真发,降级为"模拟端到端测试"
> - **下一步需用户手动跑 1 封真实邮箱实测**(`--real --recipient <用户邮箱> --max-recipients 1 --confirm "yes-i-understand-this-sends-real-email"`)
> - **D5.7 docs 收口延后**:等真实 1 封实测通过后再启动(避免"未验证的代码进 docs")
...
- ✅ **D5.6.1**(本步,commit `pending`):5 项检查员反馈修复(P0 凭证注入 + P1.1 措辞降级 + P1.2 防误发 4 重 + P1.3 审批契约 + P2 失败断言)
- ⏸️ **D5.6.2**(下一步,需用户手动):1 封真实邮箱实测(`--real --recipient user@xxx --max-recipients 1 --confirm ...`)
- ⏸️ **D5.7 docs 收口**:等 D5.6.2 真实实测通过后再启动
...
**D5.6.1 修复 5 项全部完成,8 质量门待重跑。需用户手动跑 D5.6.2 真实 1 封实测,才能判定 D5.6 真正完成 + B3 解封 + 启动 D5.7 docs 收口。**
...
python scripts/spike_send_100.py \
  ...
  --smtp-password "<QQ授权码,从 Keychain 读>" \
...
- 2. 备用邮箱收到 1 封测试邮件
- 3. 邮件内容 = "Spike Send Subject 1" + "这是 D5.6.1 spike 的第 1 封合成邮件正文..."

**任一不通过**:检查 SMTP 配置 / Keychain 凭证 / 网络 / 防火墙,修复后重跑。

### 8.2 D5.7 docs 收口(等 D5.6.2 通过后启动)
...
**D5 业务调度器 v1.0 真正锁定条件**:D5.6.2 真实 1 封实测通过 + 8 件套全部完成 + 8 质量门 8/8 全绿 + 跨项目 memory 同步。

---

**最后更新**:2026-06-13(D5.6.1 修复收口,撤回 B3 解封)
**D5 状态**:D5.1-D5.5 全部完成,D5.6.1 模拟 spike 通过,D5.6.2 真实 1 封实测待用户手动
**B 类延后**:B1/B2/B4/B6/B7/B8(6 项)+ B3 撤回解封(等 D5.6.2)
**维护者**:Mr-PRY
**当前模型**:MiniMax-M3
